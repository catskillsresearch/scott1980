...
## 2026-07-06 (continued 8) — Exercise 8.21: Partial, `λa.a§` constructed via `gMap`, isomorphism deferred
...
`D_{a§} ≅ (D_a)§`." New file `Scott1980/Neighborhood/Exercise821.lean`.

**Key realization:** Scott's "fixed-point construction" for `a§` is *literally* Exercise 7.17 Part
2's catamorphism `gMap` (already fully built and proved computable in `Exercise717Part2.lean`), no
new recursion machinery needed. `a§` should satisfy `a§(x§)=(a x)§` and `a§⟨y,z⟩=⟨a§ y,a§ z⟩` —
exactly `gMap`'s two defining equations (`gMap_in`/`gMap_pair`) specialized to `u := in∘a`,
`v := pair`:
```
aSharpInner a := gMap (inSharpMap.comp a) pairSharpMap hU : U§ → U§
aSharp a      := iSharp ∘ aSharpInner a ∘ jSharp          : U → U
```
where `iSharp/jSharp : U§ ⇄ U` (`dsharpU_trianglelefteq_U`) is a computable projection pair fixed
from `theorem_8_8_b_strong` applied to `dsharpPresentation`, exactly Definition 8.9's own recipe for
fixing `i₊/j₊` etc.

**What's proved:** `U§ ⊴ U` (computable pair `iSharp`/`jSharp`); `aSharp a` computable whenever `a`
is (`aSharp_isComputable`, chaining `gMap_isComputable` through `iSharp`/`jSharp`'s computability);
`a ≤ I ⟹ a§ ≤ I` (`aSharp_le_idMap_of_le`, half of "`a` a projection ⟹ `a§` a projection", Prop
8.10's `a ⊑ I ⟹ a+b ⊑ i∘j ⊑ I` recipe transcribed to `§`). This needed two new general tools not
previously in the codebase: **`gMap_mono`** (monotonicity of the catamorphism `gMap` in `u,v`, by
direct structural induction on the `GRel` derivation, parallel to `gRel_mono`) and
**`gMap_inSharp_pairSharp_eq_idMap`** (`gMap in pair = I_{D§}`, by two structural inductions — on
`GRel` and on `MemS` — mirroring `gMap_in`/`gMap_pair`'s own proof style). Both are fully
choice-free (`⊆ {propext, Quot.sound}`).

**What's deferred (exactly analogous to `Proposition810.lean`'s own documented omission of Prop
8.10's second half):**
1. **Idempotence** `a = a∘a ⟹ a§ = a§∘a§` (the other half of "`a` projection ⟹ `a§` projection").
   Unlike `×`/`→`/`+`, `D§`'s elements can be infinite trees with no elementwise closed form for
   `gMap`'s general action, so this needs a genuine **uniqueness-of-catamorphism** lemma (Scott
   asserts it: "`g` is unique because the values on finite elements are fixed") — comparable in
   size to `gMap`'s own ~250-line construction, not yet formalized.
2. **The isomorphism `D_{a§} ≅ (D_a)§` itself** — the exercise's actual headline claim. Needs (a) a
   general "conjugate a fixed-point-set across a projection pair" transport lemma, and (b) the
   deeper fact that `aSharpInner a`'s fixed points on `U§` are exactly the tree algebra generated by
   `D_a = fixedNbhd a`. Left as a documented follow-up.

`lake build` (whole project) green, zero `sorry`. Wired into `Scott1980.lean`. `arxiv.md`'s Exercise
8.21 row updated to `Partial`. Axiom audit: `dsharpU_trianglelefteq_U`/`aSharp_isComputable`/
`aSharp_le_idMap_of_le ⊆ {propext, Classical.choice, Quot.sound}` (inherited from `U`, matching
`Definition89.lean`/`Proposition810.lean`'s baseline, nothing new); `gMap_mono`/
`gMap_inSharp_pairSharp_eq_idMap ⊆ {propext, Quot.sound}`.
