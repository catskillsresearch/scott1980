...
* **Status:** Deferred — `(e)(a)`–`(c)` are `Pass`; `(e)(d)(i)`–`(ii)` are `Pass` (the genuine new mathematics — minimal-level canonicalization, fixing the `left_congr`/`right_congr` gap found in `(e)(b)`'s raw bisection — is done); `(e)(d)(iii)` (mechanical packaging/instantiation for `U`↔`V`) not started; `(d)(6)`'s `SplitSpec'`-mentioning note flagged for re-verification (see finding 1)
...
* **Status:** Deferred — `(e)(d)(i)`–`(ii)` Pass, `(e)(d)(iii)` not started
