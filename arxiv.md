...
* **Status:** Partial — 8.12(d)(1)–(2) are `Pass`; (d)(3)–(6) remain `Deferred`, awaiting confirmation on the design decision above before executing further
...
* **Proof Notes:** state = a pair of `P₀`/`P₁` indices, updated alternately via (2)'s computable splits, mirroring `Theorem88d.lean`'s `atomUCodeState`/`atomStep`, plus the per-step correctness theorem connecting it back to (1)'s abstract `atomPair`.
* **Status:** Deferred
