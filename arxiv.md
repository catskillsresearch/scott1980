...
* **Status:** Partial — construction `Pass` (conditional correctness only, exactly as `(c)`); closure sub-goals below: `(d)(i)`–`(d)(ii)` are `Pass`, `(d)(iii)`–`(d)(vi)` remain `Scoped`, not started
