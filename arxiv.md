...
* **Status:** Partial — construction `Pass` (conditional correctness only, exactly as `(c)`); closure sub-goals below: `(d)(i)`–`(d)(v)` are `Pass`, only `(d)(vi)` remains `Scoped`, not started
