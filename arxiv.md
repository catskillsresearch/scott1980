...
* **Status:** Partial — construction `Pass` (conditional correctness only, exactly as `(c)`); closure sub-goals below: `(d)(i)`–`(d)(iv)` are `Pass`, `(d)(v)`–`(d)(vi)` remain `Scoped`, not started
