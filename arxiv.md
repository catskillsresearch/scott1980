...
* **Status:** Partial — construction `Pass` (conditional correctness only); closure sub-goals below (dropping the "found" hypothesis): `(c)(i)`/`(c)(ii)` are `Pass`, `(c)(iii)`–`(c)(vi)` `Scoped`, not started
