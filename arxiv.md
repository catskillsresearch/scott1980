...
* **Status:** Partial (8.12(c)(i)–(vi) — Boolean-closure infra, generic definitions, generalized splitting lemma, the core two-sided atom invariant, pairwise disjointness, and all of the bidirectional transfer/glue layer — are `Pass`; 8.12(c)(vii) — final assembly — remains `Deferred`, not started — see sub-rows)
