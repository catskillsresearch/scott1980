...
* **Status:** Partial — construction `Pass` (conditional correctness only, exactly as `(c)`); closure sub-goals below: `(d)(i)`–`(d)(iii)` are `Pass`, `(d)(iv)`–`(d)(vi)` remain `Scoped`, not started
