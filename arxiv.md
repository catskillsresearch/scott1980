...
* **Lean File:** `Scott1980/Neighborhood/Exercise723.lean`
* **Proof Notes:** **Three of the four parts done and audited; `fun`/`graph` (the reflexive-domain combinators) not yet mechanised — see below.** `nbhd n ⊆ nbhd k ↔ myLor n k = n` (`nbhd_subset_iff_myLor_eq`) is the master reduction: every binary combinator here tests `Eₖ ⊆ h(Eₙ,Eₘ)`, i.e. `nbhd n ⊆ nbhd k` reindexed, so **`∩`/`∪`** (`capMap`/`cupMap`, `capMap_isComputable`/`cupMap_isComputable`) reuse `PNpres.incl_computable` directly with zero new bitwise machinery. **`λx,y.x+y`** (Minkowski sum `x+y=\{n+m\|n∈x,m∈y\}`, `plusMap`/`plusMap_isComputable`) needed real bit-level work: `bitAt`/`primrec_bitAt` (`Nat.testBit` made primitive-recursive via `halfIter`), `orUpTo`/`plusIdx` (`plusIdx n m` = iterative bitwise-OR of `m<<<a` over set bits `a` of `n`, mirroring `myLor`'s fold), `compl_nbhd_plusIdx : (nbhd(plusIdx n m))ᶜ = (nbhd n)ᶜ +ˢ (nbhd m)ᶜ`, and a `plusStep`/`Nat.Primrec.prec` primitive-recursive presentation (`primrec_plusIdx`). **Computable elements of `PN`** (`isComputableElement_iff_elemSet_re`): `elemSet x := ⋃\{Eₙ\|x.mem(nbhd n)\}` identifies `PN.Element ≃o (Set ℕ,⊆)`; `nbhd_mem_iff_subset_elemSet : x.mem(nbhd n) ↔ Eₙ⊆elemSet x` (⟸ via a choice-free finite-covering lemma `exists_combined_witness`, combining per-bit witnesses with `myLor`); forward direction is a direct r.e. projection; converse packages `Eₙ⊆elemSet x` as a bounded conjunction over a primitive-recursive coded list (`bitsCode`/`primrec_bitsCode`, mirroring `plusIdx`'s iteration) via `REPred.forall_mem_decodeList` — **the headline fact that PN's computable elements are exactly the r.e. sets**, Scott's classical result for the powerset domain. All of the above is audited **fully choice-free `⊆{propext,Quot.sound}`** (`#print axioms` on every top-level theorem) — this took real care: several `simp`/`omega`/`simpa`/`by_contra` calls silently pulled in `Nat.unpair_pair`, `Set.compl_subset_compl`, or `Set.compl_inter` (general classical `BooleanAlgebra` lemmas even though the `Set ℕ`-specific fact is constructive), fixed by hand-rolled choice-free replacements (`compl_subset_compl_of_subset`, `compl_inter_nbhd`, `nbhd_subset_iff_compl_subset_compl` — the last using a `Bool` case-split instead of the classical contrapositive) and by rewriting two `theorem foo (n : ℕ) : ∀ k, … \| 0 => … \| k+1 => …` equation-compiler recursions as `induction k with` (the pattern-match form's `simp`-closed base case reached for `Nat.unpair_pair`). **Not yet done: `fun`/`graph`.** Sketch for the next session: since `PN.Element ≃o Set ℕ` (as above) matches `Pω`'s `(Set ℕ,⊆)` exactly, `fun`/`graph` should reuse Exercise 5.14's `tag`/`entries`/`Fun`/`Graph` machinery, but built as genuine `ApproximableMap`s (`graph : ApproximableMap (funSpace PN PN) PN`, `fun : ApproximableMap PN (funSpace PN PN)`) using `funPresentation`'s coded entry-lists `el : List ℕ` (`Theorem75.lean`'s `funListOf`/`funPair`, each entry `e` coding a step pair `(nbhd e.unpair.1, nbhd e.unpair.2)`). The key correspondence: `f.rel (nbhd a) (nbhd b)` (i.e. `Eₐ⊆elemSet x → E_b⊆elemSet(f x)`) plays exactly the role of Exercise 5.14's `entries ns ⊆ x → m∈f(x)` clause with `ns := bitsList a` (so `entries (bitsList a) = Eₐ` exactly, `mem_bitsList` already proven here); `graphMap.rel W (nbhd k)` should hold iff `Eₖ ⊆ ⋃\{tag (bitsList e.unpair.1) m \| e∈el, m∈E_{e.unpair.2}\}` for `W = stepFun(funListOf PNpres PNpres el)`, and `funMap.rel (nbhd n) Z` (dually) iff every `e∈el` has `E_{e.unpair.2} ⊆ \{m\|∃ns, entries ns⊆E_{e.unpair.1} ∧ tag ns m∈Eₙ\}`. Proving the five `ApproximableMap` axioms (`rel_dom`,`rel_cod`,`master_rel`,`inter_right`,`mono`) against `funSpace`'s own (nontrivial) `mono`/`inter_right` and then `IsComputableMap` is a substantial standalone effort (comparable in size to `Theorem75.lean`'s `eval`/`curry` machinery) — left for a dedicated session.
* **Status:** Partial — `∩`/`∪`/`+` computable and the computable-elements characterization done and audited choice-free; `fun`/`graph` not yet mechanised (see proof notes for the concrete plan)
