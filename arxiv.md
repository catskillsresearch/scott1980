...
* **Status:** Partial (1 of 4 sub-parts Pass; foundational recursion + per-step correctness + the `atomUCode` invariant done, see sub-rows)
...
* **Lean File:** `Scott1980/Neighborhood/Theorem88d.lean` (not yet attempted)
* **Proof Notes:** Build via a bounded fold with `unionUX` (already built, `Theorem88d.lean`), in the style of `Recursive.lean`'s existing `bExistsFn`/`bForallFn` bounded folds. Prove the closed-form correctness using (vii)(1)'s pairwise-disjointness clause for the "no other atom leaks a point in" half, mirroring `split_fst_eq_inter_Yseq`/`atomU_succ_eq` (`Theorem88.lean`, ~lines 384–450). Depends on 8.8(b)(vii)(1).
* **Status:** Not Yet
...
* **Lean File:** `Scott1980/Neighborhood/Theorem88d.lean` (not yet attempted)
* **Proof Notes:** Should mirror `Theorem88a.lean`'s `DprimeU`/`theorem_8_8_a` assembly shape, and `Theorem88c.lean`'s `DprimeUPresentation` shape for the `ComputablePresentation` half. Depends on 8.8(b)(vii)(2).
* **Status:** Not Yet
...
* **Lean File:** `Scott1980/Neighborhood/Theorem88d.lean` (not yet attempted)
* **Proof Notes:** Once `D''`/`YseqCode` are in hand, `rel (X n) (UX m) ↔ UX (YseqCode P n) ⊆ UX m`, a *decidable* (not just r.e.) predicate in `(n, m)` — check for an existing `subsetUChar`-style decider in `DAtomDecidable.lean`/`Recursive.lean`'s interval machinery before building a new one; decidability trivially implies `REPred₂`, satisfying `Definition72.lean`'s `IsComputableMap`. Depends on 8.8(b)(vii)(3).
* **Status:** Not Yet
