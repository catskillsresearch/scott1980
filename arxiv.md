...
* **Status:** Partial (8.12(a),(b) — `V` itself and its computable presentation — are `Pass`; 8.12(c) is itself split into 7 sub-parts (c)(i)–(c)(vii), with (i)–(v) `Pass` and (vi)–(vii) `Deferred`; 8.12(d)–(g) remain `Deferred`, none yet started — see sub-rows)
...
* **Status:** Partial (8.12(c)(vi)(1)–(4) `Pass`; 8.12(c)(vi)(5) `Partial` (mem-or-∅ facts and sub-part (a) `Pass`; sub-parts (b)–(d) `Deferred`); 8.12(c)(vi)(6) merged into (5); 8.12(c)(vi)(7) remains `Deferred` — see sub-rows)
...
* **Status:** Partial (mem-or-∅/subset-of-master facts `Pass`; the order/intersection transfer layer now scoped into sub-parts (a)–(d), with (a) `Pass` and (b)–(d) `Deferred` — see sub-rows)
...
* **Proof Notes:** planned as a single induction on `n` proving the even- and odd-depth statements together (each full step's even case needs the previous step's odd case, and vice versa), using (a)'s two closed-form lemmas as the two half-step rewrites — mirrors `atomU_eq_genAtom`'s one-line inductive step, just performed twice (once per half-step) per level of `n` instead of once.
* **Status:** Deferred
...
* **Lean File:** `Scott1980/Neighborhood/Exercise812c.lean` (not yet started)
* **Proof Notes:** planned as pure assembly, no new mathematical content: given arbitrary `δ' : ℕ → Bool`, de-interleave it into `δ k := (δ' (2*k), δ' (2*k+1)) : ℕ → Bool × Bool` (re-interleaving recovers `δ'` exactly, i.e. `combinedδ δ = δ'`, by (b)'s definitions); case-split `n`'s parity — the even case reduces via (b) to `atomPair_invariant`'s clause (a) (already `Pass`, (iv)); the odd case reduces via (b) to `xStep_spec_bit`'s matching-emptiness clause (already `Pass`, (vi)(4)(c)). No disjointness or new invariant needed (unlike `Theorem88.lean`'s `atomU_invariant`, whose disjointness clause is only needed internally by (vi)(4)'s already-proved `XPseq`/`YPseq` I-formulas, not again here).
* **Status:** Deferred
...
* **Lean File:** `Scott1980/Neighborhood/Exercise812c.lean` (not yet started)
* **Proof Notes:** planned as direct instantiation of `Theorem88.lean`'s already-generic, non-`private` `transfer_dir`/`transfer_empty_iff`/`transfer_subset_iff`/`transfer_inter_empty_iff`/`transfer_double_subset_iff`/`transfer_inter_eq_iff` with `Z1 := combinedX`, `M1 := D₀.master`, `Z2 := combinedY`, `M2 := D₁.master`, and (c)'s `hcore` — exactly mirroring how (vi)(2)'s `Exercise812cYseq.lean` reused these lemmas verbatim for the abstract `E` case. The only new work is specializing the resulting even-index/odd-index statements (e.g. `transfer_subset_iff`'s conclusion at indices `2i,2j` and separately at `2i+1,2j+1`) back down to plain statements about `X`/`XPseq` and `Y`/`YPseq` by unfolding `combinedX (2i) = X i`/`combinedX (2i+1) = YPseq i` (definitionally, from (b)) — expected to be routine `rfl`/`simp`-level unwinding once (a)–(c) are in place, the same style as `Theorem88a.lean`'s `embed_subset_iff` specializing `transfer_subset_iff` via `idxSet`.
* **Status:** Deferred
...
* **Lean File:** — (not yet started)
* **Proof Notes:** analogous to Theorem 8.8(b)'s relationship to 8.8(a) — comparable difficulty (that refinement alone took 8 sub-parts across `Theorem88b.lean`–`Theorem88g.lean`). Needs a code-native version of (c)'s back-and-forth recursion (`Nat.Primrec`-indexed matching sequence, mirroring `Theorem88d.lean`–`Theorem88f.lean`'s `atomUCode`/`YseqCode` pattern) plus computability of the resulting projection pair in both directions.
* **Status:** Deferred
