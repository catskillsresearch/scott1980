...
* **Status:** Deferred — `(e)(a)`–`(c)` are `Pass`; `(e)(d)` re-scoped into `(i)`–`(iii)` (not started — `(i)`/`(ii)` are genuine new mathematics, a `left_congr`/`right_congr` gap found in `(e)(b)`'s bisection, see `(e)(d)`'s row); `(d)(6)`'s `SplitSpec'`-mentioning note flagged for re-verification (see finding 1)
...
* **Mathematical Target:** a presentation-independent canonical `(level, mask)` pair for any nonempty `levelSet k m`:
  - `def minLevel (k m : ℕ) : ℕ` — the smallest `k' ≤ k` such that `m` is periodic with period `2 ^ k'` within `[0, 2^k)` (i.e. `m = myUpsample k' k (m % 2 ^ k')`)
  - `def minMask (k m : ℕ) : ℕ := m % 2 ^ (minLevel k m)`
  - `theorem primrec_minLevel`/`primrec_minMask : Nat.Primrec …` (bounded search over `k' ≤ k`, mirroring `myFirstBit`'s `Nat.rec`-fold idiom)
  - `theorem levelSet_minLevel (k m : ℕ) : levelSet (minLevel k m) (minMask k m) = levelSet k m`
  - `theorem minLevel_unique {k₁ m₁ k₂ m₂ : ℕ} (h : levelSet k₁ m₁ = levelSet k₂ m₂) (hne : (levelSet k₁ m₁).Nonempty) : minLevel k₁ m₁ = minLevel k₂ m₂ ∧ minMask k₁ m₁ = minMask k₂ m₂` — the crux fact that actually delivers `left_congr`/`right_congr` in `(e)(d)(ii)`
* **Lean File:** — (not yet started); likely a new file `Scott1980/Neighborhood/MinLevel.lean` or appended to `LevelSetPrimrec.lean`
* **Proof Notes (plan):** `minLevel k m` via a bounded search `k' = 0, 1, …, k` (first `k'` — searching upward, mirroring `myFirstBit`'s "smallest witness" idiom exactly — for which the periodicity check `∀ i < 2^k, m.testBit i = m.testBit (i % 2^k')` holds; always terminates by `k' = k` itself, trivially periodic). `levelSet_minLevel` should follow from repeated application of `levelSet_myUpsample` once periodicity is unfolded into an explicit `myUpsample` equation. `minLevel_unique` is the hard part: reduce both sides to the common level `J = max k₁ k₂` via `levelSet_eq_iff_myUpsample_eq` (already `Pass`, `LevelSetPrimrec.lean`), then argue that two *already-minimal* (non-further-periodic) masks whose upsamples to `J` agree must already have been equal at a common level — likely by strong induction on `J - minLevel`, peeling off one upsample step at a time and using periodicity-preservation under a single de-upsample step.
...
* **Lean File:** — (not yet started); `Scott1980/Neighborhood/SplitV.lean` (revised) or a new file, whichever avoids disrupting `(e)(b)`'s already-`Pass` declarations unnecessarily
* **Proof Notes (plan):** since `minLevel_unique` forces `n`, `n'` presenting the same set to route through the *identical* `(minLevel, minMask)` pair, `splitVBit`/`splitVUpsampled` (recomputed from that pair) are then literally equal for `n`, `n'`, making `splitVLeft_congr`/`splitVRight_congr` close by `congrArg` once the pipeline is rerouted — the genuinely new content is entirely in `(e)(d)(i)`, not here.
* **Status:** Scoped, not started — depends on `(e)(d)(i)`
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** mechanical once `(e)(d)(i)`–`(ii)` land: `B812e` is a direct field-by-field assembly; `isComputableSplit_812e` is a one-line `isComputableSplit_ofBisection` instantiation fed `U_isPositive`/`U_noMinimal`/`U_diffClosed` (`Exercise812c.lean`, already `Pass`).
* **Status:** Scoped, not started — depends on `(e)(d)(i)`–`(ii)`; completes `8.12(e)` in full
