...
* **Status:** Partial — construction `Pass` (conditional correctness only); closure sub-goals below (dropping the "found" hypothesis): `(c)(i)`–`(c)(iii)` are `Pass`, `(c)(iv)`–`(c)(vi)` `Scoped`, not started
