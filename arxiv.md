...
(inventory rows **7.22a–h**, all **Pass**); what remains (**7.22i–l**) is **integration** into
...
| C11 | Infinite-word equations (prose) | ☑ | 7.22h |
| C12 | Inventory + axiom audit | ☑ | — |
| **C9a** | First missing **generic** `Nat.Primrec` lemma in `Recursive.lean` | ☐ | 7.22i |
| **C9b** | `primrec_ssysConsChar` + `Ssys_cons_computable` instantiation | ☐ | 7.22i |
...
Scott's **Exercise 7.22** is split below into sub-rows **7.22a–l** (proven blocks first, then open
items with plans). Composer sessions **C1–C8**, **C11**, **C12** delivered **7.22a–h**; **C9a–C10**
and optional **C7b** deliver **7.22i–k**.
...
* **Lean File:** `Scott1980/Neighborhood/Exercise722.lean` (docstring)
* **Proof Notes:** Composer **C11** prose: `σ⃗σ⃗=σ⃗` YES; `σ⃗σ⃗σ⃗=σ⃗` YES; `σ⃗1⃗σ⃗1⃗=σ⃗1⃗` YES; `01⃗01⃗01⃗01⃗=01⃗01⃗` NO. Not formalized in Lean (see **7.22l**).
* **Status:** Pass (documented answers; not mechanized)

#### Exercise 7.22i
* **Mathematical Target:** primitive-recursive certification of consistency — `RecDecidable₂` for Def 7.1 (ii)
* **Lean File:** `Scott1980/Neighborhood/Recursive.lean` (generic lemmas), `Exercise722Presentation.lean` (instantiation)
* **Proof Notes:** Mathematics done (`ssys_cons_char_iff`). **`ssysConsChar`**, **`Ssys_cons_computable_of_primrec_ssysConsChar`**, **`RecDecidable₂.of_paired_zero_one_char`** in place. **Open:** `primrec_ssysConsChar : Nat.Primrec ssysConsChar` and hence **`Ssys_cons_computable`**. **Plan (Composer C9a→C9b):** (C9a) prove the **first missing generic** primrec closure in `Recursive.lean`—fuel-bounded decode and/or structural `{0,1}` folds via existing `foldCode`/`existsListChar`/`primrec_selectFn`, not a monolith duplicating `SExpr` encode/decode; (C9b) short instantiation `primrec_ssysConsChar` + `Ssys_cons_computable := …`. Avoid unfolding deep iff chains at WHNF (shallow char lemmas only).
* **Status:** Not Yet

#### Exercise 7.22j
* **Mathematical Target:** `ComputablePresentation Ssys` / `Ssys.IsEffectivelyGiven` (Def 7.1 packaging)
* **Lean File:** `Scott1980/Neighborhood/Exercise722Presentation.lean`
* **Proof Notes:** **Plan (Composer C10, after 7.22i):** define **`SsysPres : ComputablePresentation …`** with enumeration `SsysX`, **`cons_computable := Ssys_cons_computable`**, and primrec `inter` if available; relation (i) **`interEq_computable`** only if **7.22k** done—else **`Ssys_partially_effectively_given`** carrying (ii) only (mirror **`Example78.lean`** / **`Exercise715.lean`** pattern).
* **Status:** Not Yet
...
* **Proof Notes:** **C7a** documents gap: emptiness insufficient (`sigma_ne_containsZero`); (i) = language equivalence (`interEq_iff`). **Plan (Composer C7b, optional):** symmetric difference via **`complDFA`** + **`toNFA`**, or Myhill–Nerode bisimulation on `autState`; does **not** block **7.22i–j** or the paper.
* **Status:** Not Yet

#### Exercise 7.22l
* **Mathematical Target:** formalize infinite-word equations in Lean (extension of Scott's investigations)
* **Lean File:** — (not started; would extend `Exercise722.lean`)
* **Proof Notes:** **Plan:** define total elements `σ⃗` as filters `{Z\|InS Z ∧ ∀n, σⁿ∈Z}`; prove or refute `mulElem` equations cited in **7.22h**. Optional; not required for Def 7.1.
* **Status:** Not Yet
...
* [Exercise722Presentation.lean](https://github.com/catskillsresearch/scott1980/blob/main/Scott1980/Neighborhood/Exercise722Presentation.lean) — `SsysX` enumeration, `ssysConsChar`, C9 `RecDecidable₂` bridge (partial)
...
* **Appendix A** — Exercise 7.22 Composer autorun (7.22a–l tracker; C9a–C9b split)
