...
* **Status:** Partial — `(d)(5)(a)`–`(e)` (zero/master facts, the interleaving/order layer, cross-family order/equality facts, `toD1Code`/`toD0Code`, and the order isomorphism `domainIsoCode812d`/`isomorphic_812d`) are all `Pass`; `(d)(5)(f)` (the exercise's literal computability target) remains not started; `(d)(4)`/`(d)(6)`'s stale `domainIso812c`-reuse notes corrected (see finding 1 and the pointers on those rows)
...
* **Status:** Partial — re-scoped into 4 sub-parts below; `(f)(i)` Pass, `(f)(ii)`–`(iv)` not started
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** per the parent row's finding 2 — `IsComputableMap P₀ P₁ f := REPred₂ (fun n m => f.rel (P₀.X n) (P₁.X m))` (Definition 7.2); via `(f)(i)`'s `toD1Code_rel_iff`, reduces to `REPred₂ (fun n m => ∃ k, P₁.X m = P₁.X (XPseqCode … k) ∧ P₀.X n ⊆ P₀.X k)`. Build via `Definition72.lean`'s `comp_isComputable`/`apply_isComputableElement` pattern (**not** `isoInj_isComputableMap`'s simpler single-step shape, since the `∃ k` here is genuine, unlike `eIdx`'s deterministic witness): `RecDecidable (fun s => P₁.X s.unpair.1 = P₁.X (XPseqCode … s.unpair.2))` via `RecDecidable.comp` reindexing `P₁.eq_computable` along the primitive-recursive `fun s => Nat.pair s.unpair.1 (XPseqCode … s.unpair.2)` (`primrec_XPseqCode`, `(d)(4)(c)`); `.re` to lift both this and `P₀.incl_computable` to `REPred`; reindex each along a primitive-recursive projection out of a shared 3-tuple encoding `(n, m, k)` (mirroring `comp_isComputable`'s `hgf`/`hgg`); `.and` to conjoin; `.proj` to close the `∃ k`; `REPred.of_iff` to match the exact target shape.
* **Status:** Scoped, not started — depends on `(f)(i)`
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** exact mirror of `(f)(i)` with `P₀`/`P₁`, `D₀`/`D₁`, `toD1Code`/`toD0Code`, `XPseqCode`/`YPseqCode` swapped throughout: `domainIsoCode812d.symm.toFun = domainIsoCode812d.invFun = toD0Code …`, so the same `ofIso`/`mem_principal` unfolding applies verbatim, mirroring `Theorem88n.lean`'s `isoProj_rel_iff_incl`.
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** exact mirror of `(f)(ii)` via `(f)(iii)`'s `toD0Code_rel_iff`, `YPseqCode`/`primrec_YPseqCode` (already established, `(d)(4)(c)`'s sibling), `P₀.eq_computable`, `P₁.incl_computable`, same `comp_isComputable`-style `REPred.and`/`.proj` assembly.
* **Status:** Scoped, not started — depends on `(f)(iii)`
