...
* **Proof Notes:** `sub f := retractionOfSubsystem (fixedNbhd_subsystem f)` — Scott's formula *is* Prop 8.2 applied to Thm 8.5's `fixedNbhd f`. **Clause 1 (range = finitary projections), both directions, complete:** `sub_le : sub f≤f`; `fixedNbhd_sub : fixedNbhd(sub f)=fixedNbhd f` (witness `Y⊆Y'⊆Y⟹Y=Y'`) giving the sharper `sub_sub : sub(sub f)=sub f` (equality, not just Scott's stated `⊑`); `sub_mono`; easy containment `isFinitaryProjection_of_sub_eq_self`; and now the hard containment `sub_eq_self_of_isFinitaryProjection : IsFinitaryProjection f→sub f=f`, unblocked by Thm 8.5's hard direction — `⊇` is `sub_le`, `⊆` unwinds `XfZ` via `rel_iff_mem_principal` into `Z∈f(↑X)` and rewrites via `formula_of_isFinitaryProjection` into exactly `sub_rel`'s shape — packaged as `sub_eq_self_iff_isFinitaryProjection`, with `isFinitaryProjection_sub : IsFinitaryProjection(sub f)` for any `f` as a corollary. **Clause 2, half done (`namespace Sub8_6`):** `subApprox : ApproximableMap(funSpace E E)(funSpace E E)` realizes "`f↦sub(f)` is itself approximable" via Exercise 2.13's `ofContinuous`, applied to `subFilter := toFilter∘sub∘toApproxMap` (i.e. `sub` transported along `funSpaceEquiv`). This needed one new general lemma, added to `Exercise213.lean`: `continuous_of_monotone_iSupDirected` (monotone + directed-sup-preserving ⟹ topologically continuous), proved directly from algebraicity (`eq_iSupDirected_principal`, a duplicate of Thm 8.5's own copy kept local to avoid a heavy import) rather than a topological-basis argument. `subFilter`'s directed-sup-preservation (`subFilter_iSupDirected`) needed *no* consistency argument: directed unions of *filters* correspond, under `toApproxMap`, to the raw union of the underlying maps' *relations* (`toApproxMap_rel_iSupDirected`, immediate from `mem_iSupDirected`), and `sub`'s formula (`sub_rel`) is a *positive* existential in `f`'s relation, so it commutes with such unions by pure logic — swapping the order of two existentials (`sub_toApproxMap_iSupDirected`). `IsRetraction subApprox`/`subApprox≤idMap` (`isProjection_subApprox`) then drop out of `sub_sub`/`sub_le` pointwise via `toElementMap_subApprox`; proved via `le_antisymm` on `le_iff_toElementMap_le` rather than the classical `ext_of_toElementMap`, to stay choice-free. **Deferred**: `IsFinitary subApprox` — every other `IsFinitary` witness in Lecture VIII is built from an *explicit* subsystem via `retractionOfSubsystem`, but that's circular here (would need Thm 8.5's hard direction applied to `subApprox` itself); the honest witness looks to need a fresh domain of "subsystems of `E`", i.e. the not-yet-formalized universal-domain machinery (Def 8.7 onward). Clause 3 (computability) sits on top of that plus `E` effectively given. **Choice-free**: all of clause 1 and the projection half of clause 2 (`#print axioms` on `sub_eq_self_iff_isFinitaryProjection`, `isProjection_subApprox`, etc.) is `⊆{propext,Quot.sound}`.
* **Status:** Partial (clause 1 fully Pass; clause 2 half Pass — approximable + projection done, finitary clause deferred; clause 3 deferred)
