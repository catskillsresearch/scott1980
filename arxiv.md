...
* **Status:** Partial (8.12(c)(i)–(v) — Boolean-closure infra, generic definitions, generalized splitting lemma, the core two-sided atom invariant, and pairwise disjointness — are `Pass`; 8.12(c)(vi) is `Partial` (4 of 7 sub-parts `Pass`); 8.12(c)(vii) — final assembly — remains `Deferred`, not started — see sub-rows)
...
* **Status:** Partial (8.12(c)(vi)(1)–(4) `Pass`; 8.12(c)(vi)(5)–(7) remain `Deferred` — see sub-rows)
