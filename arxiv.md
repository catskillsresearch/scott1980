...
* **Proof Notes:** Split exactly along Scott's own hint ("it is easy to check that `Π(d)` is a projection; the problem is to show it is *finitary*") — see sub-rows **8.27(a)** (Pass) and **8.27(b)** (Open, with its own sub-plan **8.27(b)(0)–(b)(5)**) below.
* **Status:** (a) Pass. (b) Open — see sub-rows
...
* **Lean File:** — (not yet started); umbrella — see sub-rows **8.27(b)(0)–(b)(5)** below
* **Proof Notes:** Unlike every finitary-closure result elsewhere in the project (`finitaryProjection_arrowComb`/`_prodComb`/`_sumComb` in `Proposition810b.lean`, Thm 8.6(b)'s own `isFinitary_subApprox`), which exhibit `Fix` as isomorphic to an *already-available* domain built from finitely many fixed ingredients, `Fix(piD d)` is a genuine *dependent* product `Π_{t:𝒰} Fix(D_{d(t)})` over a continuum-indexed, dependently-varying family — a new piece of domain theory (logical-relations/parametricity-flavoured) with no template in Lectures I–VIII and, distinctively among all ~200 exercises/theorems in this book, **no hint given at all** for this half. Also addresses, in `Exercise827.lean`'s module docstring (no separate lemma needed), Scott's parenthetical "why does this equation mean `x` is in the product?" — `x(t)=d(t)(x(t))` unwinds via Ex 8.26's `Uapply` to `x(t)∈Fix(D_{d(t)})`, i.e. `x` is a genuine dependent section. **Key strategic pivot for the plan below:** rather than constructing a fresh dependent-product `NeighborhoodSystem` ex nihilo, target **Theorem 8.5**'s step-closure formula (ii) for `a := piD d` directly (`Theorem85.lean`'s `finitaryProjection_iff_formula`, already proved as a genuine iff — the `(ii)⟹(i)` direction `isFinitaryProjection_of_formula` needs *only* the formula, no external witness domain, and automatically supplies `fixedNbhd (piD d)` as the correct witness). This turns "invent new domain theory" into "compute with `piD d`'s own `.rel`," matching the shape of every other finitary-closure proof in the project. Full subgoal breakdown in `HANDOFF.md`'s 2026-07-07 checkpoint and in sub-rows (b)(0)–(b)(5) below; a fallback (explicit coded dependent-product construction, mirroring `Definition79`/`Exercise715`'s recipe) is documented there too, to be attempted only if the formula-(ii) route stalls.
* **Status:** Open
...
* **Lean File:** — (not yet started)
* **Proof Notes:** `isFinitaryProjection_of_formula (piD d) (b)(3)` then `(b)(0)` gives `IsFinitaryProjection (piU d)`, combining with the already-Pass 8.27(a) (`isProjection_piU`, itself a special case not needing polymorphism).
* **Status:** Planned

#### Exercise 8.27(b)(5)
* **Mathematical Target:** Bookkeeping: build, axiom-audit, and close out the `arxiv.md`/`HANDOFF.md` status for 8.27(b).
* **Lean File:** — (not yet started)
* **Proof Notes:** `lake build`, `#print axioms` (expect `⊆{propext,Classical.choice,Quot.sound}`, matching the rest of `𝒰`-based material — not a new choice source). Flip 8.27(b)'s row to Pass; consider merging 8.27(a)/(b) back into a single `Pass` `Exercise 8.27` row once both halves are closed. Dated `HANDOFF.md` checkpoint. **Fallback, only if (b)(2)/(b)(3) hit a genuine wall:** if `curry`'s abstract `.rel` doesn't unwind cleanly, or formula (ii) turns out not to literally hold for `piD d`, fall back to an explicit dependent-product construction (token type = finite lists of `(type-nbhd-code, fiber-nbhd-code)` pairs, ranging over `subU`'s and each `D'_s`'s own generating bases via their `fixedNbhd` witnesses), mirroring `Definition79`/`Proposition710`'s power-domain recipe or `Exercise715`'s `D^∞` recipe but *dependent* rather than uniform — a strictly larger undertaking (roughly a fresh Definition+Proposition pair), documented in `HANDOFF.md`.
* **Status:** Planned
