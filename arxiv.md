...
* **Status:** Partial (3 of 4 sub-parts Pass; foundational recursion + per-step correctness + the `atomUCode` invariant + `YseqCode`'s closed form + the `D''` assembly/isomorphism/subsystem/presentation done, see sub-rows; only `IsComputableMap` itself, (vii)(4), remains)
