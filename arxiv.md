...
* **Status:** Partial — 8.12(d)(1)–(2) are `Pass`; (d)(3) has been re-scoped into 6 sub-parts (`(d)(3)(a)–(f)`, see below), **all now `Pass`** — 8.12(d)(3) is complete; (d)(4) has been re-scoped into 4 sub-parts (`(d)(4)(a)–(d)`, see below), **all now `Pass`** — `(d)(4)(c)`'s and `(d)(4)(d)`'s own nested closures (`(c)(i)`–`(c)(vi)` and `(d)(i)`–`(d)(vi)` respectively) are both `Pass`, unconditional, so 8.12(d)(4) is complete; (d)(5)–(6) remain `Deferred`, with cross-reference notes added from `(d)(4)`'s scoping investigation
...
* **Status:** Deferred — re-scoped into 6 sub-parts below (none started); `(d)(4)`/`(d)(6)`'s stale `domainIso812c`-reuse notes corrected (see finding 1 and the pointers on those rows)
...
* **Status:** Design decision resolved; re-scoped into 4 sub-parts below (`(b)(i)` Pass, rest not started)
...
* **Mathematical Target:** the code-level analogue of `Exercise812c.lean`'s `yStep_fst_eq_inter_YPseq`/`yStep_fst_succ_eq`, needed to seed `(b)(iii)`'s `combinedXCode`'s odd-depth half-step. Structurally harder than `(b)(i)` (matching `(d)(4)(d)`'s own "genuinely harder" finding for `YPseqCode` vs `XPseqCode`): `yPseqAtomIdx n i bx` has a *free* extra bit `bx ∈ {0, 1}` (the inner `xSubStep`'s own bit), and `(d)(5)(b)(i)`'s helper `xPseqAtomIdx0_eq` (`(d)(5)(a)`) only covers the `bx = 1` case (where it happens to coincide with `xPseqAtomState`) — the `bx = 0` case needs a *new* closed form off `xSubStep_idx0_eq`'s general `selectFn bx (P₀.inter …) (hDiff0.diffIdx …)` shape, and the disjointness argument now needs *two* independent sources of distinctness (different `i` below `n`, **or** the same `i` but different `bx` — the latter cheap: `A ∩ Xn`/`A \ Xn` are disjoint as raw sets, no `SplitSpec'` needed).
* **Lean File:** — (not yet started)
* **Status:** Scoped, not started — depends on `(b)(i)`'s technique (distinctness lemma reusable verbatim); new content is the `bx = 0` closed form and the two-source disjointness case split

##### Exercise 8.12(d)(5)(b)(iii): `combinedXCode`/`combinedYCode`/`combinedδCode`, and `hcore`
* **Mathematical Target:** the generalized interleaved-family machinery, mirroring `Exercise812c.lean`'s `combinedX`/`combinedY`/`combinedδ` (lines 1236–1457) with `atomPair → atomPairG`, `splitChoice' → splitX/splitY`, `XPseq k → P₁.X (XPseqCode … k)`, `YPseq k → P₀.X (YPseqCode … k)`:
  - `combinedXCode(2k) := P₀.X k`, `combinedXCode(2k+1) := P₀.X (YPseqCode … k)`; `combinedYCode(2k) := P₁.X (XPseqCode … k)`, `combinedYCode(2k+1) := P₁.X k`; `combinedδCode δ (2n) := (δ n).1`, `combinedδCode δ (2n+1) := (δ n).2`.
  - Odd-depth half-step identities `genAtom_combinedXCode_succ_eq`/`genAtom_combinedYCode_succ_eq` (the `combinedXCode`-through-`X`-substep case is elementary, no I-formula needed, mirroring `genAtom_combinedX_succ_eq`; the `combinedYCode`-through-`X`-substep case needs `(b)(i)`'s I-formula's `succ`-shaped two-branch closed form, analogous to `xStep_snd_succ_eq`, not yet built — a thin wrapper over `(b)(i)`'s headline fact plus the algebraic "false branch from `SplitSpec'`'s `∪`/`∩ = ∅`" argument `xStep_snd_succ_eq`/`yStep_fst_succ_eq` both use).
  - `atomPairG_fst_eq_genAtomCode`/`atomPairG_snd_eq_genAtomCode` (induction on `n`, using `(b)(ii)`'s I-formula's own `succ`-shaped closed form for the *other* family's odd step, symmetric to the `X`-side).
  - `hcore_even`/`hcore_odd`/`hcore` (via `deinterleaveδ`/`combinedδCode_deinterleaveδ`, `atomPairG_invariant`'s clause (a), and `xStepG_spec`'s matching-emptiness clause — both already `Pass`, no new content).
* **Lean File:** — (not yet started)
* **Status:** Scoped, not started — depends on `(b)(i)`, `(b)(ii)`
...
* **Lean File:** — (not yet started)
* **Status:** Scoped, not started — depends on `(b)(iii)`
...
* **Status:** Scoped, not started — depends on `(d)(5)(b)`
...
* **Status:** Scoped, not started — depends on `(d)(5)(a)`, `(d)(5)(c)`
...
* **Status:** Scoped, not started — depends on `(d)(5)(d)`
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** unfold `ofIso`/`toD1Code`/`V.principal` exactly as `isoInj_rel_iff_incl` does, landing the `∃ hX, …` on the stated `∃ k, …` shape (dropping `hX` since `P₀.mem_X n` is automatic). The outer `∃ k` is r.e. (`REPred`) over a decidable body: `P₁.X m = P₁.X (XPseqCode … k)` is `P₁.eq_computable` (Definition 7.1, generic on any `ComputablePresentation`, needs no new proof) reindexed along the already-`Nat.Primrec` `XPseqCode` (`primrec_XPseqCode`, `(d)(4)(c)`) via `RecDecidable.of_iff`, conjoined (`RecDecidable.and`) with `P₀.incl_computable`; `REPred.of_iff` then closes it, exactly mirroring `isoInj_isComputableMap`'s proof shape line-for-line. Symmetric for `toD0Code`/`YPseqCode`/`P₀.eq_computable`/`P₁.incl_computable`. **Once this lands, `(d)(5)` is complete and `(d)(6)` (final `EffectiveIso` assembly) becomes close to immediate**, per `(d)(6)`'s own (corrected) proof note.
* **Status:** Scoped, not started — depends on `(d)(5)(e)`
...
* **Lean File:** — (not yet started); new file `Scott1980/Neighborhood/SplitV.lean`, mirroring `SplitU.lean`'s structure
* **Proof Notes (plan):** direct transcription of `V_no_minimal`'s proof (`Exercise812.lean`), which is already fully constructive — the only missing piece is a primitive-recursive "first set bit below `2^k`" finder (`myFirstBit k m`, a bounded search mirroring `LevelSetPrimrec.lean`'s existing `bExistsFn`-style combinator, or a fresh bounded `Nat.rec` scan) standing in for `V_no_minimal`'s classical `ℓ₀` witness (`levelSet_nonempty_iff.mp hne`). Given `myFirstBit`, `splitVLeft n`/`splitVRight n` compute `pair (k+1) (2^ℓ₀)` / `pair (k+1) (myUpsample k (k+1) m ^^^ 2^ℓ₀)` via `LevelSetPrimrec.lean`'s existing `myUpsample` plus a new (standard) primitive-recursive `xor`/pow-of-2. `splitV_disjoint`/`splitV_union` transcribe `V_no_minimal`'s `hInter`/`hUnion` computations verbatim.
