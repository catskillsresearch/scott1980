...
* **Status:** Partial — re-scoped into 3 sub-parts below; `(d)(6)(a)` Pass, `(d)(6)(b)`–`(c)` not started
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** exact mirror of `(a)` (independent of it), mirroring `Theorem88n.lean`'s `isoInj_comp_isoProj`: same shape with `e`/`e.symm` swapped and `OrderIso.apply_symm_apply` in place of `OrderIso.symm_apply_apply`.
* **Status:** Scoped, not started — depends on `(d)(5)`
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** per the parent row's finding 3 — `toMap`/`invMap`/`toMap_computable`/`invMap_computable`/`left_inv`/`right_inv` are all direct citations of `(d)(5)(f)`'s `domainIsoCode812d_isComputableMap`/`domainIsoCode812d_symm_isComputableMap` and `(a)`/`(b)`'s `invMap_comp_toMap`/`toMap_comp_invMap`; `effectivelyIsomorphic_812d` is a one-line `⟨effectiveIso812d …⟩` packaging, exactly mirroring `(d)(5)(e)(iv)`'s `domainIsoCode812d`/`isomorphic_812d` assembly pattern. Once this lands, `8.12(d)(6)` — hence `8.12(d)` in full — is complete.
* **Status:** Scoped, not started — depends on `(a)`/`(b)`
