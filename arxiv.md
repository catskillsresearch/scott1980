...
**7.22l**). The only remaining (optional) extension is upgrading the `ConsistencyPresentation`
already proved to a full `ComputablePresentation` via `inter`/`inter_primrec`/`inter_spec`/
`masterIdx`. We mechanize Scott's least positive neighbourhood system generated by
