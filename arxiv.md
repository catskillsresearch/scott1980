...
* **Proof Notes (plan):** direct transcription of `XPseq_zero`/`YPseq_zero`'s proof strategy — unfold `atomPairG` one step at `n = 0` (its `0`-clause is `(D₀.master, D₁.master)` regardless of sign-history, mirroring `atomPair`'s own), use `hxSplit`/`hySplit`'s `SplitSpec'` fields (the "`-`"-branch forced empty since `D₀.master \ P₀.X 0 = ∅` from `hX0`) to pin the "`+`"-branch to the full master, then transport through `mem_XPseqCode_iff_unconditional`/`mem_YPseqCode_iff_unconditional` (`(d)(4)(c)(vi)`/`(d)(4)(d)(vi)`, already `Pass`) at `n = 0`, `i = 0` (the unique bit-source `< 4⁰ = 1`). Per finding 4, expected **not** to need the interleaving layer (`(d)(5)(b)`) at all — should be provable independently and first.
