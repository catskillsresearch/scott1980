...
* **Proof Notes:** Unblocked by (vi)'s resolution: Part 6 now provides a genuine `ComputablePresentation` of `D'` (`DprimeUPresentation`, `Theorem88c.lean`) alongside `U`'s own (Part 3, `UComputablePresentation.lean`), so `IsComputableMap` for `Subsystem.inj`/`Subsystem.proj` (`DprimeU_subsystem`, `D' ◁ U`) can be attacked directly via `Definition72.lean`'s `IsComputableMap` against these two presentations — likely mirroring the `fixMap_isComputable`/Theorem 8.6(c) r.e.-predicate idiom (bounded search over `U`'s presented intervals) rather than needing any `atomUCode`/`DomainIso`→`ApproximableMap` bridge.
* **Status:** Not Yet
