...
* **Lean File:** `Scott1980/Neighborhood/Theorem88.lean` (in progress)
* **Proof Notes:** Scott's back-and-forth construction: enumerate `D = {Xₙ∣n∈ℕ}` (`X₀=Δ`), and recursively choose `Yₙ∈U` so that for every `δ∈{+,-}ⁿ` the "atom" `⋂ᵢ δᵢXᵢ` is empty iff `⋂ᵢ δᵢYᵢ` is empty (`(■)`). **Key local step, proved and reusable** (`exists_split`): given a matching atom pair `(A,B)` (`A=∅↔B=∅`, `B=∅∨U.mem B`) and a new target `Xₙ`, produce two new matching atom pairs for `A∩Xₙ`/`A\Xₙ` — the only nontrivial case (both `A∩Xₙ` and `A\Xₙ` nonempty) invokes **Definition 8.7's `U_no_minimal`** directly: split `B` into disjoint proper nonempty `U`-neighbourhoods `Y,Z` and take `I:=Y` (so `B\I=Z` is free, no interval-difference-closure lemma needed at all — the three cases are handled entirely by `∅`, `B` itself, or `U_no_minimal`'s output). Scott's `(♦)`/`𝒟≅𝒟†` preparation is needed **only** for the *effective* decidability of atom-emptiness (Thm 8.8(b)), not for this general Prop-level existence argument (which case-splits classically), so this part skips the dagger/positivity machinery entirely. **Remaining:** package the per-atom local step into a `List`-indexed recursive construction of the full sequence `Y : ℕ → Set ℚ` (avoiding `Fin n → Bool` dependent bookkeeping via a doubling `List (Set α × Set ℚ)` of atoms), derive `Xᵢ⊆Xⱼ ↔ Yᵢ⊆Yⱼ` from the atom-emptiness invariant, and assemble the final `D ≅ᴰ D'`/`D' ◁ U` package. `Classical.choice` is expected and acceptable here (a genuinely non-constructive `Prop`-level existence statement for arbitrary countable `D`).
* **Status:** In progress

#### Theorem 8.8(b)
* **Mathematical Target:** if `D` is effectively given, the projection pair witnessing `D ⊴ U` can be taken computable
* **Lean File:** — (Formalization deferred, blocked on 8.8(a))
* **Proof Notes:** the effective refinement of 8.8(a)'s construction: replace the classical case-splits with `D`'s own decidable presentation (via the `𝒟≅𝒟†` preparation making atom-emptiness decidable, `(♦)`), and replace `U_no_minimal`'s existential witness with an explicit, primitive-recursive interval-splitting formula (`U`'s own computable presentation from Definition 8.7).
* **Status:** Deferred
