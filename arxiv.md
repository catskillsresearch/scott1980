...
* **Status:** Partial (8.12(c)(vi)(1)–(3) `Pass`; 8.12(c)(vi)(4) `Partial` (sub-part (a) `Pass`, (b)–(d) `Deferred`); 8.12(c)(vi)(5)–(7) remain `Deferred` — see sub-rows)
...
* **Lean File:** `Scott1980/Neighborhood/Exercise812c.lean`
* **Proof Notes:** unlike `X n`'s side, `yStep`'s own inputs (`A1`, `B1`, the *post*-`X`-sub-step values) already depend on `(δ n).1` itself, not just history strictly below `n` — so `YPseq`'s union needs an *extra* free `Bool` parameter `bx` (for position `n`'s `X`-sub-step bit) alongside the usual `Fin n → Bool × Bool` history, i.e. `YPseq n := ⋃ (δ' : Fin n → Bool × Bool) (bx : Bool), (yStep D₀ hD₀nomin B1 A1 (Y n) true).1` where `A1`/`B1` are `xStep`'s outputs at bit `bx` applied to `atomPair (extendTruePair δ') n`. `subset_YPseq` is the resulting (doubly-indexed) `Set.subset_iUnion` fact, exactly as trivial as `subset_XPseq`.
* **Status:** Deferred

#### Exercise 8.12(c)(vi)(4)(c)
* **Mathematical Target:** A reusable bridging lemma: for any `δ : ℕ → Bool × Bool`, `n`, and target bit `bx : Bool`, produce a concrete `δ'' : ℕ → Bool × Bool` agreeing with `δ` strictly below `n` with `(δ'' n).1 = bx`, together with the transport facts (`atomPair`-congruence, `xStep_spec` instantiated at `δ''`) needed to connect an arbitrary `(δ', bx)` witness of `YPseq n` back to a concrete history
* **Lean File:** `Scott1980/Neighborhood/Exercise812c.lean`
* **Proof Notes:** mirrors `Theorem88.lean`'s `Function.update`-based `δ2`/`δ3` device inside `split_fst_eq_inter_Yseq` (needed there because `atomU`'s recursion threads `δ n`'s own value into its depth-`n+1` step) — expected to use `Function.update (extendTruePair δ') n (bx, true)` (second component irrelevant/arbitrary) plus `extendTruePair_restrictFinPair_agree`-style agreement lemmas and `atomPair_congr`.
* **Status:** Deferred
...
* **Lean File:** `Scott1980/Neighborhood/Exercise812c.lean`
* **Proof Notes:** assembles (vi)(4)(b)+(vi)(4)(c) with `xStep_spec`/`yStep_fst_subset`/`xStep_disjoint_of_ne`/`xStep_fst_subset`/`atomPair_disjoint` (all already `Pass`, (v)(2)/(v)(3)/(iv)), by the same two-sided antisymmetry argument as `xStep_snd_eq_inter_XPseq` — but needing the (vi)(4)(c) bridge for the `⊇` direction's "history agrees below `n` but `bx ≠ (δ n).1`"/"bit-realization" case, unlike `XPseq`'s simpler proof.
* **Status:** Deferred
