...
* **Status:** Partial — construction `Pass` (conditional correctness only); closure sub-goals below (dropping the "found" hypothesis): `(c)(i)`–`(c)(iv)` are `Pass`, `(c)(v)`/`(c)(vi)` `Scoped`, not started
