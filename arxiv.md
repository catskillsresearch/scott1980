...
* **Status:** Partial — 8.12(d)(1) is `Pass`; (d)(2)–(6) remain `Deferred`, awaiting confirmation on the design decision above before executing further
...
* **Proof Notes:** a new `Prop`/structure, mirroring `IsComputableMap`'s two-presentation shape: a primitive-recursive index function taking `(n : index of A-piece, m : index of B)` to indices of the two output pieces, correct against `P₀.X`/`P₁.X`.
* **Status:** Deferred
