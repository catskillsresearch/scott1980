...
Lecture VIII covers retractions, projections, and the construction of the universal domain $U$. The retraction/projection spine (Definitions 8.1/8.3, Proposition 8.2, Example 8.4(a), Theorem 8.5's easy direction, Theorem 8.6's `sub` combinator) is formalized below; the universal domain `U` (Def 8.7 onward), Example 8.4(b), and a few other hard/large items remain deferred.
...
* **Proof Notes:** **Strategy (not yet started).** Both combinators reuse 8.4(a)'s `check`/`fade` machinery at a higher type: `strict` needs `fade:O×E→E` composed with the *evaluation*/`curry` structure of `funSpace D E` (comparable in size to `Theorem75.lean`'s step-neighbourhood work — `ofMono`/`curry`-style lifting of a first-order combinator to `funSpace`'s neighbourhoods is not yet in the codebase); proving "range = strict functions is a domain" additionally needs `IsFinitary`/`IsFinitaryProjection` (Def 8.3, already available) applied to the resulting subsystem. `smash` is more self-contained (built from `prod`/`paired`/`check`/`fade` exactly as in 8.4(a), just nested twice — first fading through `x`'s check, then through `y`'s check on the pair `⟨x,y⟩`) and is likely the more tractable of the two to formalize first; showing its range is isomorphic to Def 3.9's smash product `D⊗E` (not yet cross-checked against the codebase's `Exercise510`/`Exercise627` smash-product infrastructure) is the remaining work. Both are retained here (rather than dropped) because they are Scott's explicit stated claims under "EXAMPLES 8.4", not merely suggested follow-ups.
* **Status:** Deferred
