...
* **Status:** Partial — `(d)(5)(a)`–`(d)` (zero/master facts, the interleaving/order layer, cross-family order/equality facts, and `toD1Code`/`toD0Code`) are all `Pass`; `(d)(5)(e)`–`(f)` (the order isomorphism and its computability) remain not started; `(d)(4)`/`(d)(6)`'s stale `domainIso812c`-reuse notes corrected (see finding 1 and the pointers on those rows)
...
* **Status:** Partial — `(e)(i)` `Pass`; `(e)(ii)`–`(iv)` not started
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** via `Element.ext`, reducing to `∀ S, (∃ m n, S = P₀.X (YPseqCode … m) ∧ P₁.X m = P₁.X (XPseqCode … n) ∧ x.mem (P₀.X n)) ↔ x.mem S`. Forward (`⟸`, building the witness from `x.mem S`): `x.sub`/`P₀.surj` names `S` as some `P₀.X n`; take `m := XPseqCode … n` (handed over for free, no search); `P₁.X m = P₁.X (XPseqCode … n)` is `rfl`; `(d)(5)(c)(iii)`'s `XPseqCode_eq_Y_iff_X_eq_YPseqCode n (XPseqCode … n) |>.mp rfl` gives `S = P₀.X (YPseqCode … m)` for free (self-referential pair, exactly `(d)(5)(d)(i)`'s `toD1Code_up_mem` pattern). Backward (`⟹`, consuming a witness `⟨m, n, hS, hmn, hxn⟩`): `XPseqCode_eq_Y_iff_X_eq_YPseqCode n m` transports `hmn` into `P₀.X n = P₀.X (YPseqCode … m)`, combined with `hS` gives `S = P₀.X n`, closing via `hxn`. Expected comparably short to `toD1Code_up_mem`/`toD0Code_up_mem` — no `hXcover`/`hYcover`-style double search, unlike `domainIso812c.left_inv`'s own proof.
* **Status:** Scoped, not started — depends on `(d)(5)(c)`, `(d)(5)(d)`
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** exact mirror of `(e)(ii)` with `P₀`/`P₁`, `XPseqCode`/`YPseqCode`, `X`/`Y` swapped throughout: `y.sub`/`P₁.surj` names the target as some `P₁.X n`; `m := YPseqCode … n` handed over for free; closes via `XPseqCode_eq_Y_iff_X_eq_YPseqCode`'s other side at the self-referential pair `(YPseqCode … n, n)`.
* **Status:** Scoped, not started — depends on `(d)(5)(c)`, `(d)(5)(d)`
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** `toFun`/`invFun`/`left_inv`/`right_inv` are direct citations of `(d)(5)(d)`'s `toD1Code`/`toD0Code` and `(e)(ii)`/`(e)(iii)`'s standalone lemmas; `map_rel_iff'` is discharged inline (not worth its own row, per finding 2) — `mpr` direction pure unfolding, `mp` direction one `P₀.surj` call (replacing `domainIso812c.map_rel_iff'`'s `hXcover`) plus `(e)(i)`'s `X_eq_iff_XPseqCode_eq`, direct transcription of `Exercise812c.lean`'s `domainIso812c.map_rel_iff'` (lines 2094–2108) otherwise. `isomorphic_812d` is a one-line `⟨domainIsoCode812d …⟩` packaging, mirroring `isomorphic_812c`.
* **Status:** Scoped, not started — depends on `(e)(i)`–`(iii)`
