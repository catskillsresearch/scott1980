...
* **Proof Notes:** Audit Exercise 7.22 Bool stack vs existing **`foldCode`** / **`existsListChar`** / **`primrec_selectFn`** infrastructure. Prove the **first missing generic** primrec lemma reusable across exercises—fuel-bounded decode and/or structural `{0,1}` folds. **Do not** duplicate `SExpr` encode/decode in a monolith. Blocks **7.22i(b)**.
* **Status:** Not Yet
...
* **Proof Notes:** Mathematics done (`ssys_cons_char_iff`). **`ssysConsChar`**, **`Ssys_cons_computable_of_primrec_ssysConsChar`**, **`RecDecidable₂.of_paired_zero_one_char`** in place. **Open:** `primrec_ssysConsChar : Nat.Primrec ssysConsChar` and hence **`Ssys_cons_computable`**. **Needs 7.22i(a).** Short instantiation using C9a lemmas; avoid unfolding deep iff chains at WHNF (shallow char lemmas only).
* **Status:** Not Yet
