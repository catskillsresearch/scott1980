...
* **Proof Notes:** Needs substantially more infrastructure than (a): a general "conjugate fixed-point-sets across a projection pair" lemma (`Fix(i∘f∘j) ≅ Fix(f)` for a projection pair `i,j`, relating `Fix(a*b)` back to `Fix(a)*Fix(b)` through the six fixed maps of Definition 8.9), plus `prodEquiv`/`funSpaceEquiv`/a coproduct-element characterization for `sum` to identify the resulting isomorphism with the expected domain construct. Left as a documented follow-up; see `HANDOFF.md`.
* **Status:** Deferred
