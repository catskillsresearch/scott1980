...
* **Proof Notes (plan, subject to the design decision flagged on the `(d)(5)` row above):** **Route 1 (safe fallback):** full generalized transcription of `Exercise812c.lean`'s `combinedX`/`combinedY`/`combinedδ` (interleave `P₀.X`/`YPseqCode` at even/odd raw indices, resp. `P₁.X`/`XPseqCode`), the `atomPairG`-as-`genAtom`-over-the-interleaved-family identities (mirroring `atomPair_fst_eq_genAtom`/`atomPair_snd_eq_genAtom`, needing `xStepG`/`yStepG` half-step closed forms analogous to `xStep_snd_succ_eq`/`yStep_fst_succ_eq` — not yet built, would need to be added here too), `deinterleaveδ`/`hcore`, and `transfer_subset_combined`/`transfer_inter_eq_combined` (mirroring (c)(vi)(5)(b)–(vi)(6), ~300 lines of precedent). **Route 2 (bounded search first, recommended):** attempt a direct argument straight from `atomPairG_disjoint`/`atomPairG_invariant`/`xStepG_snd_union` plus `(d)(4)(c)`/`(d)(4)(d)`'s `mem_XPseqCode_iff_unconditional`/`mem_YPseqCode_iff_unconditional` (both directions are set-membership biconditionals already, so a subset/intersection fact might transport through them directly without an interleaving detour) — cap the search effort and fall back to Route 1 if no clean argument surfaces. **Expected the single largest chunk of `(d)(5)`, plausibly warranting its own further sub-numbering (`(b)(i)`–`(b)(n)`) once a route is chosen and execution starts**, mirroring how `(d)(4)(c)`'s own closure needed 6 further sub-parts.
* **Status:** Scoped, not started — blocked on the design decision on the `(d)(5)` row
