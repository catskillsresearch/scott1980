...
* **Lean File:** `Scott1980/Neighborhood/Exercise812.lean` (`V` itself); umbrella — see sub-rows **8.12(a)–8.12(f)** below for the `U≅V` isomorphism plan
* **Proof Notes:** **`V` itself is fully formalized (no `sorry`, `Pass`-quality).** `affine k ℓ := {n \| n%2^k=ℓ}` (Scott's `2^kℕ+ℓ`, auto-`∅` for `ℓ≥2^k`). Bookkeeping-free encoding `levelSet k m := {n \| m.testBit(n%2^k)}` (a single bitmask replaces `Definition87.lean`'s `List`, since residue classes at a fixed level partition `ℕ`, unlike arbitrary rational intervals). `upsample k k' m` re-expresses a level-`k` mask at finer level `k'` via `Nat.equivBitIndices` (`ℕ≃Finset ℕ` bit-index bijection), giving `levelSet_upsample`; `levelSet_inter` (upsample-both-then-`&&&`) shows closure under intersection **unconditionally** (mirrors `Example78.lean`'s `PN`, unlike `U` which needs a consistency hypothesis). `V : NeighborhoodSystem ℕ` from these. `V_no_minimal` (Scott's Def-8.7 remark transplanted): refine one level finer so every witnessing bit `ℓ₀` doubles into `(ℓ₀,ℓ₀+2^k)`, peel off one copy, the twin guarantees the "rest" is non-empty. `V_mem_iff_finiteUnion` (faithfulness, mirrors `U_mem_iff_scott`): via `V_union_mem`/`V_iUnion_mem` (mirrors `U_union_mem`/`U_iUnion_mem`) reassembling Scott's literal finite unions from `levelSet`. **Why `U≅V` is hard (not just tedious):** order isos preserve compactness, and compact = `principal` (general facts, `Theorem85.lean`), so `U≅V` would force `principal_U([1/3,2/3))` (a genuine compact `U`-element, non-dyadic endpoints) to correspond to a *single* `V`-neighbourhood — impossible, since no finite union of `2^kℕ+ℓ` equals `[1/3,2/3)` exactly. So "restrict a `U`-filter to its dyadic trace" is provably the wrong map, not just hard to compute. The isomorphism is nonetheless very likely true via a genuinely different technique: a **computable back-and-forth** matching of the full (richly-overlapping, atomless — `U_no_minimal`/`V_no_minimal`) neighbourhood posets, analogous to the classical fact that countable atomless dense structures of the same signature are isomorphic. **Broken into a 6-part plan** (mirroring how Theorem 8.8 was split into 15 files/~4,470 lines across (a)/(b)/(c)), tracked as sub-rows **8.12(a)–8.12(f)**, all currently **Deferred** (none yet started): (a) `V`'s `ComputablePresentation` — tractable, self-contained, no back-and-forth needed; (b) a new general non-effective back-and-forth lemma (two countable atomless neighbourhood systems, each satisfying a mutual one-step extension property relative to the other, are order-isomorphic) — genuinely new abstract infrastructure, since Theorem 8.8(a)'s `exists_split`/`Theorem88.lean` apparatus is one-sided (only ever needs `U`'s own `U_no_minimal`) and does not directly generalize to a two-sided match; (c) the effective refinement of (b) (extension property witnessed computably ⟹ `EffectiveIso`), analogous in difficulty to Theorem 8.8(b)'s own 8-part refinement of 8.8(a); (d) `U` satisfies the extension property relative to `V` (splitting matched `V`-pieces finely enough via density of `ℚ` — this is where the `[1/3,2/3)` obstruction above must actually be confronted and resolved, not sidestepped); (e) `V` satisfies the symmetric extension property relative to `U` (splitting matched `U`-intervals via density of dyadic rationals — expected easier than (d), since `V`'s dyadic pieces are coarse and `U`'s dense intervals can approximate them arbitrarily well); (f) assembly of (b)/(c) via (d)/(e) into the concrete `U≅ᴰV` effective isomorphism, closing the exercise. See sub-rows for full detail, and the file's module docstring / `HANDOFF.md` for the original single-pass writeup this plan refines.
* **Status:** Partial (`V` fully formalized, `Pass`-quality; the `U≅V` isomorphism broken into 6 deferred sub-parts 8.12(a)–(f), none yet started — see sub-rows)

#### Exercise 8.12(a)
* **Mathematical Target:** Part 1 of 6 — `V`'s effective presentation (`ComputablePresentation V`), mirroring `UComputablePresentation.lean`
* **Lean File:** — (not yet started)
* **Proof Notes:** self-contained, no back-and-forth needed — the most tractable of the 6 parts and can go green independently of (b)–(f). Needs `Nat.Primrec` bit-manipulation infrastructure for `upsample`/`levelSet`: an effective enumeration of `(k,m)` pairs representing non-empty `levelSet`s (`V`'s neighbourhoods), decidable membership (`n ∈ levelSet k m` is already plain `Nat.testBit`, so this should be easy), and a computable intersection operation matching `levelSet_inter`'s `upsample`-then-`&&&` formula (the missing "upsample a bitmask from level `k` to level `k'`, computably" piece flagged in `Exercise812.lean`'s docstring).
* **Status:** Deferred

#### Exercise 8.12(b)
* **Mathematical Target:** Part 2 of 6 — new general (non-effective) lemma: two countable, atomless neighbourhood systems `D₀`,`D₁`, each satisfying a mutual one-step "extension"/splitting property relative to the other, are order-isomorphic (`D₀.Element ≃o D₁.Element`)
* **Lean File:** — (not yet started)
* **Proof Notes:** the genuinely new piece of abstract theory this exercise needs. Theorem 8.8(a)'s `exists_split`/`Theorem88.lean` back-and-forth is one-sided (embeds an arbitrary `D` into `U`, only ever splitting `U`'s side via `U_no_minimal`); this lemma needs a *symmetric* two-sided version, alternately enumerating `D₀`'s and `D₁`'s neighbourhoods and extending a finite matching by splitting whichever side is "behind", needing *both* systems' no-minimal-neighbourhood property. Expected to reuse the `List`-of-matched-pairs recursive idiom from `Theorem88.lean` but symmetrized. `Classical.choice` expected and acceptable (non-constructive `Prop`-level existence, analogous to Theorem 8.8(a)).
* **Status:** Deferred

#### Exercise 8.12(c)
* **Mathematical Target:** Part 3 of 6 — effective refinement of (b): if `D₀`,`D₁` are effectively given and (b)'s extension property is witnessed computably, the resulting order isomorphism is an `EffectiveIso`
* **Lean File:** — (not yet started)
* **Proof Notes:** analogous to Theorem 8.8(b)'s relationship to 8.8(a) — comparable difficulty (that refinement alone took 8 sub-parts across `Theorem88b.lean`–`Theorem88g.lean`). Needs a code-native version of (b)'s back-and-forth recursion (`Nat.Primrec`-indexed matching sequence, mirroring `Theorem88d.lean`–`Theorem88f.lean`'s `atomUCode`/`YseqCode` pattern) plus computability of the resulting projection pair in both directions.
* **Status:** Deferred

#### Exercise 8.12(d)
* **Mathematical Target:** Part 4 of 6 — `U` satisfies (b)/(c)'s extension property relative to `V`
* **Lean File:** — (not yet started)
...
#### Exercise 8.12(e)
* **Mathematical Target:** Part 5 of 6 — `V` satisfies (b)/(c)'s extension property relative to `U`
* **Lean File:** — (not yet started)
* **Proof Notes:** the symmetric combinatorics on the dyadic-residue side: given a finite matching plus one new `V`-neighbourhood `levelSet k m`, show it extends by splitting the matched `U`-side intervals finely enough via density of dyadic rationals in `ℚ`. Expected to be the easier direction of (d)/(e), since `V`'s own neighbourhoods are already "coarse" dyadic pieces that `U`'s dense rational intervals can approximate arbitrarily well.
* **Status:** Deferred

#### Exercise 8.12(f)
* **Mathematical Target:** Part 6 of 6 — assembly: apply (b)/(c)'s general lemma to the concrete `U`,`V` using (d)/(e)'s extension-property proofs to conclude `U ≅ᴰ V` effectively, closing out Exercise 8.12
* **Lean File:** — (not yet started)
* **Proof Notes:** expected to need no new mathematical content beyond instantiating (b)/(c) with (d)/(e) — analogous to how `theorem_8_8_a`/`theorem_8_8_b` assemble the general apparatus for the specific case at hand.
* **Status:** Deferred
