...
* **Status:** Partial (8.12(c)(vi)(1)–(4) `Pass`; 8.12(c)(vi)(5) `Partial` (mem-or-∅ facts and sub-parts (a)/(b) `Pass`; sub-parts (c)/(d) `Deferred`); 8.12(c)(vi)(6) merged into (5); 8.12(c)(vi)(7) remains `Deferred` — see sub-rows)
...
* **Status:** Partial (mem-or-∅/subset-of-master facts `Pass`; the order/intersection transfer layer now scoped into sub-parts (a)–(d), with (a)/(b) `Pass` and (c)/(d) `Deferred` — see sub-rows)
