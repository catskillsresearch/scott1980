...
* **Status:** Scoped, not started; re-scoped into 4 sub-parts below (none started)
...
* **Mathematical Target:** the `D₀.Element → D₁.Element` pushforward map itself:
  - `def toD1Code (x : D₀.Element) : D₁.Element where mem T := ∃ n, T = P₁.X (XPseqCode … n) ∧ x.mem (P₀.X n); …`
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** `sub := fun ⟨n, hn, _⟩ => hn ▸ XPseqCode_mem_unconditional n` (`(d)(4)`, already `Pass`); `master_mem := ⟨0, (XPseqCode_zero …).symm, by rw [hX0]; exact x.master_mem⟩` (`(d)(5)(a)`, already `Pass`); `inter_mem` per the parent row's finding 3 — `rintro T1 T2 ⟨i, rfl, hxi⟩ ⟨j, rfl, hxj⟩; exact ⟨P₁.inter i j, (P₁.inter_spec ⟨_, x.sub (x.inter_mem hxi hxj)⟩).symm, x.inter_mem hxi hxj⟩` directly, no `exists_inter_index_X`-style helper needed; `up_mem := toD1Code_up_mem x`.
* **Status:** Scoped, not started — depends on `(d)(5)(d)(i)`
...
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** exact mirror of `(d)(5)(d)(i)`: destructure `h1` as `⟨i, rfl, hyi⟩`; `P₀.surj hD0S2` names `S2` as some `P₀.X k`; `(d)(5)(c)(ii)`'s `YPseqCode_subset_X_iff_Y_subset_XPseqCode i k` turns `hS1S2` into `P₁.X i ⊆ P₁.X (XPseqCode … k)`; `y.up_mem hyi (XPseqCode_mem_unconditional k) this` gives `y.mem (P₁.X (XPseqCode … k))` — already `y.mem (P₁.X j)` for `j := XPseqCode … k`, no search needed; close via `(d)(5)(c)(iii)`'s `XPseqCode_eq_Y_iff_X_eq_YPseqCode k (XPseqCode … k) |>.mp rfl : P₀.X k = P₀.X (YPseqCode … (XPseqCode … k))`, i.e. `= S2`.
* **Status:** Scoped, not started — depends on `(d)(5)(c)`, `(d)(4)` (`XPseqCode_mem_unconditional`)

##### Exercise 8.12(d)(5)(d)(iv): `toD0Code`, full assembly
* **Mathematical Target:** the `D₁.Element → D₀.Element` pullback map, mirror of `(d)(5)(d)(ii)`:
  - `def toD0Code (y : D₁.Element) : D₀.Element where mem S := ∃ n, S = P₀.X (YPseqCode … n) ∧ y.mem (P₁.X n); …`
* **Lean File:** — (not yet started)
* **Proof Notes (plan):** `sub := fun ⟨n, hn, _⟩ => hn ▸ YPseqCode_mem_unconditional n`; `master_mem := ⟨0, (YPseqCode_zero …).symm, by rw [hY0]; exact y.master_mem⟩` (`(d)(5)(a)`); `inter_mem := fun ⟨i, rfl, hyi⟩ ⟨j, rfl, hyj⟩ => ⟨P₀.inter i j, (P₀.inter_spec ⟨_, y.sub (y.inter_mem hyi hyj)⟩).symm, y.inter_mem hyi hyj⟩` directly; `up_mem := toD0Code_up_mem y`.
* **Status:** Scoped, not started — depends on `(d)(5)(d)(iii)`
