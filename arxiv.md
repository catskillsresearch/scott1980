...
(inventory rows **7.22a–h**, all **Pass**); what remains (**7.22i(a)–l**) is **integration** into
...
| **C9b** | `primrec_ssysConsChar` + `Ssys_cons_computable` instantiation | ☐ | 7.22i(b) |
...
(proven blocks first, then open items with plans). Composer sessions **C1–C8**, **C11**, **C12**
delivered **7.22a–h**; **C9a–C10** and optional **C7b** deliver **7.22i(a)–k**.
...
* **Proof Notes:** **`isBinDigit`**, **`allBinDigitsChar`**, **`primrec_isBinDigit`**, **`primrec_allBinDigitsChar`** — `{0,1}` validation over **`decodeList`** via existing **`allListChar`**/`**foldCode**`. First reusable gap for **`decodeListBool`** in **7.22i(b)**. **Next for C9b:** fuel-bounded **`SExpr.decode`** char + **`decideNonemptyB`** stack. Green, zero `sorry`; primrec theorems `⊆ {propext, Quot.sound}`.
* **Status:** Pass

#### Exercise 7.22i(b)
* **Mathematical Target:** primitive-recursive certification of consistency — `RecDecidable₂` for Def 7.1 (ii) (Composer **C9b**)
* **Lean File:** `Scott1980/Neighborhood/Exercise722Presentation.lean` (instantiation; generic lemmas in `Recursive.lean`)
* **Proof Notes:** Mathematics done (`ssys_cons_char_iff`). **`ssysConsChar`**, **`Ssys_cons_computable_of_primrec_ssysConsChar`**, **`RecDecidable₂.of_paired_zero_one_char`** in place. **Open:** `primrec_ssysConsChar : Nat.Primrec ssysConsChar` → **`Ssys_cons_computable`**. **C9b strategy (2026-06-30):** one **slice per session**—do **not** rebuild the abandoned `Exercise722Primrec.lean` monolith (~840 lines, never green). Dependency chain: (1) **`decodeFuelOkChar`** + `primrec` (fuel induction via `primrec_tagCase4`, reusing **7.22i(a)**); (2) generic list ops in **`Recursive.lean`** as needed (`listLenChar`, `listEqChar`, `takeCode`/`dropCode`, `appendListCode`)—each green before the next; (3) **`matchesBChar`** / **`decideNonemptyBChar`** / **`consistentBChar`** numeric mirrors; (4) **`ssysConsistentBChar`** + shallow `_eq_one_iff` links to the Bool layer (**no** WHNF unfold of `ssys_cons_char_iff`); (5) **`primrec_ssysConsChar`** + instantiate **`Ssys_cons_computable`**. **Blocked attempt (not checked in):** a bulk WIP hit proof failures on list-append/tabulation lemmas and one `sorry` before reaching step (5). **Retry:** land generic **`Recursive.lean`** lemmas incrementally, then a short **`Exercise722Presentation.lean`** instantiation only.
* **Status:** Not Yet
...
* **Proof Notes:** **C7a** documents gap: emptiness insufficient (`sigma_ne_containsZero`); (i) = language equivalence (`interEq_iff`). **Plan (Composer C7b, optional):** symmetric difference via **`complDFA`** + **`toNFA`**, or Myhill–Nerode bisimulation on `autState`; does **not** block **7.22i(a)–j** or the paper.
* **Status:** Not Yet
...
* **Appendix A** — Exercise 7.22 Composer autorun (7.22a–h, 7.22i(a)–l tracker; C9a–C9b split)
