...
private def genAtom {β : Type*} (Z : ℕ → Set β) (M : Set β) (δ : ℕ → Bool) : ℕ → Set β
  | 0 => M
  | (n + 1) => genAtom Z M δ n ∩ (if δ n then Z n else M \ Z n)

private theorem genAtom_subset {β : Type*} (Z : ℕ → Set β) (M : Set β) (δ : ℕ → Bool) (n : ℕ) :
...
private theorem genAtom_congr {β : Type*} (Z : ℕ → Set β) (M : Set β) {δ δ' : ℕ → Bool} {n : ℕ}
...
private theorem genAtom_forward {β : Type*} {Z : ℕ → Set β} {M : Set β} {δ : ℕ → Bool} {n : ℕ}
...
private theorem genAtom_self {β : Type*} (Z : ℕ → Set β) (M : Set β) {x : β} (hx : x ∈ M) :
...
`atomU` below be defined by plain structural recursion on `ℕ`; `splitChoice_spec` recovers
...
theorem splitChoice_spec {A : Set α} {B : Set ℚ} (hAB : A = ∅ ↔ B = ∅)
    (hBU : B = ∅ ∨ U.mem B) (Xn : Set α) :
    ((splitChoice A B Xn).1 = ∅ ∨ U.mem (splitChoice A B Xn).1) ∧
      ((splitChoice A B Xn).2 = ∅ ∨ U.mem (splitChoice A B Xn).2) ∧
      (A ∩ Xn = ∅ ↔ (splitChoice A B Xn).1 = ∅) ∧
      (A \ Xn = ∅ ↔ (splitChoice A B Xn).2 = ∅) ∧
      (splitChoice A B Xn).1 ∪ (splitChoice A B Xn).2 = B ∧
      (splitChoice A B Xn).1 ∩ (splitChoice A B Xn).2 = ∅ := by
  classical
  unfold splitChoice
  rw [dif_pos ⟨hAB, hBU⟩]
  exact (exists_split hAB hBU Xn).choose_spec.choose_spec

/-- `splitChoice`'s first output is a subset of `B` (`I ⊆ B`, from `I ∪ J = B`). -/
theorem splitChoice_fst_subset {A : Set α} {B : Set ℚ} (hAB : A = ∅ ↔ B = ∅)
    (hBU : B = ∅ ∨ U.mem B) (Xn : Set α) : (splitChoice A B Xn).1 ⊆ B :=
  Set.subset_union_left.trans_eq (splitChoice_spec hAB hBU Xn).2.2.2.2.1

/-- `splitChoice`'s second output is a subset of `B` (`J ⊆ B`, from `I ∪ J = B`). -/
theorem splitChoice_snd_subset {A : Set α} {B : Set ℚ} (hAB : A = ∅ ↔ B = ∅)
    (hBU : B = ∅ ∨ U.mem B) (Xn : Set α) : (splitChoice A B Xn).2 ⊆ B :=
  Set.subset_union_right.trans_eq (splitChoice_spec hAB hBU Xn).2.2.2.2.1

/-! ### The `U`-side atoms, built via `splitChoice`

Fix the enumeration `X : ℕ → Set α` of `D` and its master `Δ := D.master`. The `D`-side atom at
depth `n` for sign sequence `δ` is simply `genAtom X Δ δ n` (no new definition needed). The
`U`-side atom `atomU X Δ δ n` is built by recursion, splitting the *previous* atom via
`splitChoice` against the *next* target `X n`, taking the `δ n`-side of the split. -/

/-- The `U`-side atom matching `genAtom X Δ δ n`, built recursively via `splitChoice`. -/
noncomputable def atomU (X : ℕ → Set α) (Δ : Set α) (δ : ℕ → Bool) : ℕ → Set ℚ
  | 0 => U.master
  | (n + 1) =>
      if δ n then (splitChoice (genAtom X Δ δ n) (atomU X Δ δ n) (X n)).1
      else (splitChoice (genAtom X Δ δ n) (atomU X Δ δ n) (X n)).2

variable (X : ℕ → Set α) (Δ : Set α)

@[simp] theorem atomU_zero (δ : ℕ → Bool) : atomU X Δ δ 0 = U.master := rfl

theorem atomU_succ (δ : ℕ → Bool) (n : ℕ) :
    atomU X Δ δ (n + 1) =
      if δ n then (splitChoice (genAtom X Δ δ n) (atomU X Δ δ n) (X n)).1
      else (splitChoice (genAtom X Δ δ n) (atomU X Δ δ n) (X n)).2 := rfl

/-- Extending/changing `δ` at or beyond position `n` does not change `atomU X Δ δ n` (mirrors
`genAtom_congr`; needs no invariant, since `splitChoice` is an ordinary function of its inputs). -/
theorem atomU_congr {δ δ' : ℕ → Bool} {n : ℕ} (h : ∀ i < n, δ i = δ' i) :
    atomU X Δ δ n = atomU X Δ δ' n := by
  induction n with
  | zero => rfl
  | succ n ih =>
    have hprev : atomU X Δ δ n = atomU X Δ δ' n := ih (fun i hi => h i (Nat.lt_succ_of_lt hi))
...
variable (hΔ : Δ.Nonempty)
include hΔ

/-- **The core invariant of Scott's back-and-forth construction**, proved by a single induction on
`n` alongside `splitChoice`'s spec: at every depth `n` and for every sign sequence `δ`,
(a) the atom-matching invariant `(■)` (`genAtom X Δ δ n = ∅ ↔ atomU X Δ δ n = ∅`),
(b) `atomU X Δ δ n` is either empty or a genuine `U`-neighbourhood, and
...
theorem atomU_invariant :
    ∀ n, (∀ δ, genAtom X Δ δ n = ∅ ↔ atomU X Δ δ n = ∅) ∧
      (∀ δ, atomU X Δ δ n = ∅ ∨ U.mem (atomU X Δ δ n)) ∧
      (∀ δ δ', (∃ i < n, δ i ≠ δ' i) → atomU X Δ δ n ∩ atomU X Δ δ' n = ∅) := by
  ...
  | succ n ih =>
    ...
    · have hspec := splitChoice_spec (ihmatch δ) (ihmem δ) (X n)
      rw [atomU_succ]
    ...
    · by_cases hagree : ∀ j < n, δ j = δ' j
      · have hδn : δ n ≠ δ' n := by
        ...
        have hB : atomU X Δ δ n = atomU X Δ δ' n := atomU_congr X Δ hagree
        have hspec := splitChoice_spec (ihmatch δ) (ihmem δ) (X n)
      ...
      · push Not at hagree
        ...
        have hd : atomU X Δ δ n ∩ atomU X Δ δ' n = ∅ := ihdisj δ δ' ⟨j, hj, hjne⟩
        have h1 : atomU X Δ δ (n + 1) ⊆ atomU X Δ δ n := by
          rw [atomU_succ]
          by_cases hδ : δ n = true
          · simp only [hδ, if_true]; exact splitChoice_fst_subset (ihmatch δ) (ihmem δ) (X n)
          · simp only [hδ]; exact splitChoice_snd_subset (ihmatch δ) (ihmem δ) (X n)
        have h2 : atomU X Δ δ' (n + 1) ⊆ atomU X Δ δ' n := by
          rw [atomU_succ]
          by_cases hδ' : δ' n = true
          · simp only [hδ', if_true]; exact splitChoice_fst_subset (ihmatch δ') (ihmem δ') (X n)
          · simp only [hδ']; exact splitChoice_snd_subset (ihmatch δ') (ihmem δ') (X n)
...
theorem atomU_succ_subset (δ : ℕ → Bool) (n : ℕ) :
    atomU X Δ δ (n + 1) ⊆ atomU X Δ δ n := by
  obtain ⟨hmatch, hmem, -⟩ := atomU_invariant X Δ hΔ n
  rw [atomU_succ]
  by_cases hδ : δ n = true
  · simp only [hδ, if_true]; exact splitChoice_fst_subset (hmatch δ) (hmem δ) (X n)
  · simp only [hδ]; exact splitChoice_snd_subset (hmatch δ) (hmem δ) (X n)
...
noncomputable def Yseq (n : ℕ) : Set ℚ :=
  ⋃ δ' : Fin n → Bool, atomU X Δ (extendTrue δ') (n + 1)

omit hΔ in
theorem subset_Yseq {n : ℕ} (δ' : Fin n → Bool) :
    atomU X Δ (extendTrue δ') (n + 1) ⊆ Yseq X Δ n :=
  Set.subset_iUnion (fun δ' => atomU X Δ (extendTrue δ') (n + 1)) δ'
...
closed-form description of the choice made by `splitChoice`. This is where pairwise disjointness
...
one. -/
theorem splitChoice_fst_eq_inter_Yseq (δ : ℕ → Bool) (n : ℕ) :
    (splitChoice (genAtom X Δ δ n) (atomU X Δ δ n) (X n)).1 = atomU X Δ δ n ∩ Yseq X Δ n := by
  obtain ⟨hmatch, hmem, hdisj⟩ := atomU_invariant X Δ hΔ n
  set A := genAtom X Δ δ n with hAdef
  set B := atomU X Δ δ n with hBdef
  set I := (splitChoice A B (X n)).1 with hIdef
  ...
  have hI_eq : atomU X Δ δ2 (n + 1) = I := by
    rw [atomU_succ]
    ...
    have hB2 : atomU X Δ δ2 n = B := by
      apply atomU_congr; intro i hi; simp [hδ2def, Function.update_of_ne (ne_of_lt hi)]
    rw [hd2n, if_pos rfl, hA2, hB2]
  have hδ3_eq : atomU X Δ δ3 (n + 1) = I := (atomU_congr X Δ hagreefull).trans hI_eq
  apply Set.Subset.antisymm
  · have hIsubB : I ⊆ B := splitChoice_fst_subset (hmatch δ) (hmem δ) (X n)
    have hIsubY : I ⊆ Yseq X Δ n := hδ3_eq ▸ subset_Yseq X Δ (restrictFin δ2 n)
  ...
  · rintro z ⟨hzB, hzY⟩
    ...
    · have hAeq : genAtom X Δ (extendTrue δ') n = A := genAtom_congr X Δ hagree
      have hBeq : atomU X Δ (extendTrue δ') n = B := atomU_congr X Δ hagree
      have hlast : extendTrue δ' n = true := by simp [extendTrue]
      have heq : atomU X Δ (extendTrue δ') (n + 1) = I := by
        rw [atomU_succ, hAeq, hBeq, hlast, if_pos rfl]
      rwa [heq] at hz'
    · push Not at hagree
      obtain ⟨j, hj, hjne⟩ := hagree
      have hzB' : z ∈ atomU X Δ (extendTrue δ') n :=
        atomU_succ_subset X Δ hΔ (extendTrue δ') n hz'
      have hempty := hdisj δ (extendTrue δ') ⟨j, hj, fun h => hjne (h.symm)⟩
      exact absurd (Set.mem_inter hzB hzB') (by rw [hempty]; simp)

theorem atomU_subset_master (δ : ℕ → Bool) (n : ℕ) : atomU X Δ δ n ⊆ U.master := by
  induction n with
  | zero => exact subset_rfl
  | succ n ih => exact (atomU_succ_subset X Δ hΔ δ n).trans ih

/-- **Closed form for `atomU`**: with `Yseq` in hand, `atomU` satisfies exactly the same
recursive description as `genAtom Yseq U.master` — the choices made by `splitChoice` are, after
the fact, forced (`splitChoice_fst_eq_inter_Yseq` handles the `true` branch; the `false` branch
follows algebraically from `I ∪ J = B`, `I ∩ J = ∅`). -/
theorem atomU_succ_eq (δ : ℕ → Bool) (n : ℕ) :
    atomU X Δ δ (n + 1) =
      atomU X Δ δ n ∩ (if δ n then Yseq X Δ n else U.master \ Yseq X Δ n) := by
  obtain ⟨hmatch, hmem, -⟩ := atomU_invariant X Δ hΔ n
  have hspec := splitChoice_spec (hmatch δ) (hmem δ) (X n)
  have hIeq := splitChoice_fst_eq_inter_Yseq X Δ hΔ δ n
  by_cases hδ : δ n = true
  · rw [atomU_succ, if_pos hδ, if_pos hδ]; exact hIeq
  · rw [atomU_succ, if_neg hδ, if_neg hδ]
    have hJeq : (splitChoice (genAtom X Δ δ n) (atomU X Δ δ n) (X n)).2 =
        atomU X Δ δ n \ Yseq X Δ n := by
      have hunion := hspec.2.2.2.2.1
      have hinter := hspec.2.2.2.2.2
      ext x
      constructor
      · intro hxJ
        have hxB : x ∈ atomU X Δ δ n := hunion ▸ Or.inr hxJ
        refine ⟨hxB, fun hxY => ?_⟩
        have hxI : x ∈ (splitChoice (genAtom X Δ δ n) (atomU X Δ δ n) (X n)).1 :=
          hIeq ▸ Set.mem_inter hxB hxY
        exact absurd (Set.mem_inter hxI hxJ) (by rw [hinter]; simp)
      · rintro ⟨hxB, hxnY⟩
        rw [← hunion] at hxB
        rcases hxB with hxI | hxJ
        · exact absurd (hIeq ▸ hxI : x ∈ atomU X Δ δ n ∩ Yseq X Δ n).2 hxnY
        · exact hxJ
    rw [hJeq]
    have hsub := atomU_subset_master X Δ hΔ δ n
    ext x
    constructor
    · rintro ⟨hx1, hx2⟩; exact ⟨hx1, hsub hx1, hx2⟩
    · rintro ⟨hx1, -, hx2⟩; exact ⟨hx1, hx2⟩
...
payoff of the whole `splitChoice`/disjointness apparatus: it lets every later argument treat
...
theorem atomU_eq_genAtom (δ : ℕ → Bool) : ∀ n, atomU X Δ δ n = genAtom (Yseq X Δ) U.master δ n
  | 0 => rfl
  | (n + 1) => by
      rw [atomU_succ_eq X Δ hΔ, atomU_eq_genAtom δ n]; rfl
...
theorem transfer_empty_iff {cs : List (ℕ × Bool)} {n : ℕ} (hn : ∀ p ∈ cs, p.1 < n) :
    {x ∈ Δ | ∀ p ∈ cs, (p.2 = true ↔ x ∈ X p.1)}.Nonempty ↔
      {y ∈ U.master | ∀ p ∈ cs, (p.2 = true ↔ y ∈ Yseq X Δ p.1)}.Nonempty := by
  have hcore : ∀ δ n, genAtom X Δ δ n = ∅ ↔ genAtom (Yseq X Δ) U.master δ n = ∅ := fun δ n =>
    (atomU_invariant X Δ hΔ n).1 δ |>.trans (by rw [atomU_eq_genAtom X Δ hΔ])
  have hcore' : ∀ δ n, genAtom (Yseq X Δ) U.master δ n = ∅ ↔ genAtom X Δ δ n = ∅ := fun δ n =>
    (hcore δ n).symm
  exact ⟨transfer_dir X Δ (Yseq X Δ) U.master hcore hn,
    transfer_dir (Yseq X Δ) U.master X Δ hcore' hn⟩
...
theorem transfer_subset_iff (i j : ℕ) :
    Δ ∩ X i ⊆ X j ↔ U.master ∩ Yseq X Δ i ⊆ Yseq X Δ j := by
  have key := transfer_empty_iff X Δ hΔ (cs := [(i, true), (j, false)]) (n := max i j + 1)
  ...
  have hLHS : {x ∈ Δ | ∀ p ∈ [(i, true), (j, false)], (p.2 = true ↔ x ∈ X p.1)}
    ...
    tauto
  have hRHS : {y ∈ U.master | ∀ p ∈ [(i, true), (j, false)], (p.2 = true ↔ y ∈ Yseq X Δ p.1)}
      = (U.master ∩ Yseq X Δ i) \ Yseq X Δ j := by
    ext y
    simp only [Set.mem_setOf_eq, List.mem_cons, List.not_mem_nil, or_false,
      forall_eq_or_imp, forall_eq, Set.mem_diff, Set.mem_inter_iff]
    tauto
...
theorem transfer_inter_empty_iff (i j : ℕ) :
    Δ ∩ X i ∩ X j = ∅ ↔ U.master ∩ Yseq X Δ i ∩ Yseq X Δ j = ∅ := by
  have key := transfer_empty_iff X Δ hΔ (cs := [(i, true), (j, true)]) (n := max i j + 1)
    (by simp only [List.mem_cons, List.not_mem_nil, or_false]
        rintro p (rfl | rfl) <;> simp [Nat.lt_succ_iff])
  have hLHS : {x ∈ Δ | ∀ p ∈ [(i, true), (j, true)], (p.2 = true ↔ x ∈ X p.1)}
    ...
    tauto
  have hRHS : {y ∈ U.master | ∀ p ∈ [(i, true), (j, true)], (p.2 = true ↔ y ∈ Yseq X Δ p.1)}
      = U.master ∩ Yseq X Δ i ∩ Yseq X Δ j := by
    ext y
    simp only [Set.mem_setOf_eq, List.mem_cons, List.not_mem_nil, or_false,
      forall_eq_or_imp, forall_eq, Set.mem_inter_iff]
...
theorem transfer_double_subset_iff (i j k : ℕ) :
    Δ ∩ X i ∩ X j ⊆ X k ↔ U.master ∩ Yseq X Δ i ∩ Yseq X Δ j ⊆ Yseq X Δ k := by
  have key := transfer_empty_iff X Δ hΔ (cs := [(i, true), (j, true), (k, false)])
    (n := max i (max j k) + 1)
    (by simp only [List.mem_cons, List.not_mem_nil, or_false]
  ...
  have hRHS : {y ∈ U.master |
      ∀ p ∈ [(i, true), (j, true), (k, false)], (p.2 = true ↔ y ∈ Yseq X Δ p.1)}
      = (U.master ∩ Yseq X Δ i ∩ Yseq X Δ j) \ Yseq X Δ k := by
    ext y
    simp only [Set.mem_setOf_eq, List.mem_cons, List.not_mem_nil, or_false,
      forall_eq_or_imp, forall_eq, Set.mem_diff, Set.mem_inter_iff]
    tauto
...
/-- `Yseq X Δ n` is always a subset of `U.master`: it is a union of `atomU`-pieces at depth `n+1`,
each of which is `⊆ U.master` by `atomU_subset_master`. -/
theorem Yseq_subset_master (n : ℕ) : Yseq X Δ n ⊆ U.master :=
  Set.iUnion_subset fun δ' => atomU_subset_master X Δ hΔ (extendTrue δ') (n + 1)

/-- **`Yseq X Δ 0 = U.master` whenever `X 0 = Δ`** (Scott's convention `X₀ = Δ`). Unfolding one step
of the recursion at `n = 0`, both `genAtom X Δ δ 0` and `atomU X Δ δ 0` are `Δ` and `U.master`
*definitionally*, so splitting against `X 0 = Δ` puts everything into the `"+"`-branch (the
`"-"`-branch `Δ \ Δ = ∅` is forced empty by `splitChoice_spec`), and `I ∪ J = U.master` with `J = ∅`
forces `I = U.master`. -/
theorem Yseq_zero_eq_master (h0 : X 0 = Δ) : Yseq X Δ 0 = U.master := by
  set δ : ℕ → Bool := extendTrue (fun i : Fin 0 => i.elim0) with hδdef
  have hδ0 : δ 0 = true := by simp [hδdef, extendTrue]
  have hAB : genAtom X Δ δ 0 = ∅ ↔ atomU X Δ δ 0 = ∅ :=
    ⟨fun h => absurd h hΔ.ne_empty, fun h => absurd h U.master_mem.2.1.ne_empty⟩
  have hBU : atomU X Δ δ 0 = ∅ ∨ U.mem (atomU X Δ δ 0) := Or.inr U.master_mem
  have hspec := splitChoice_spec hAB hBU (X 0)
  have hJ : (splitChoice (genAtom X Δ δ 0) (atomU X Δ δ 0) (X 0)).2 = ∅ := by
    rw [← hspec.2.2.2.1]
    show genAtom X Δ δ 0 \ X 0 = ∅
    rw [show genAtom X Δ δ 0 = Δ from rfl, h0]
    exact Set.diff_self
  have hI : (splitChoice (genAtom X Δ δ 0) (atomU X Δ δ 0) (X 0)).1 = U.master := by
    have hunion := hspec.2.2.2.2.1
    rw [hJ, Set.union_empty] at hunion
    exact hunion
  have hkey : atomU X Δ δ 1 = U.master := by rw [atomU_succ, hδ0, if_pos rfl]; exact hI
  apply Set.Subset.antisymm (Yseq_subset_master X Δ hΔ 0)
  calc U.master = atomU X Δ δ 1 := hkey.symm
    _ ⊆ Yseq X Δ 0 := subset_Yseq X Δ (fun i : Fin 0 => i.elim0)
...
theorem transfer_inter_eq_iff (i j k : ℕ) (hi : X i ⊆ Δ) (_hj : X j ⊆ Δ) (hk : X k ⊆ Δ) :
    X i ∩ X j = X k ↔ Yseq X Δ i ∩ Yseq X Δ j = Yseq X Δ k := by
  have h1 : X k ⊆ X i ↔ Yseq X Δ k ⊆ Yseq X Δ i := by
    have := transfer_subset_iff X Δ hΔ k i
    rwa [Set.inter_eq_self_of_subset_right hk,
      Set.inter_eq_self_of_subset_right (Yseq_subset_master X Δ hΔ k)] at this
  have h2 : X k ⊆ X j ↔ Yseq X Δ k ⊆ Yseq X Δ j := by
    have := transfer_subset_iff X Δ hΔ k j
    rwa [Set.inter_eq_self_of_subset_right hk,
      Set.inter_eq_self_of_subset_right (Yseq_subset_master X Δ hΔ k)] at this
  have h3 : X i ∩ X j ⊆ X k ↔ Yseq X Δ i ∩ Yseq X Δ j ⊆ Yseq X Δ k := by
    have := transfer_double_subset_iff X Δ hΔ i j k
    rwa [Set.inter_eq_self_of_subset_right hi,
      Set.inter_eq_self_of_subset_right (Yseq_subset_master X Δ hΔ i)] at this
  constructor
  · intro heq
  ...
  · intro heq
    have hki : Yseq X Δ k ⊆ Yseq X Δ i := heq ▸ Set.inter_subset_left
    have hkj : Yseq X Δ k ⊆ Yseq X Δ j := heq ▸ Set.inter_subset_right
    have hijk : Yseq X Δ i ∩ Yseq X Δ j ⊆ Yseq X Δ k := heq ▸ subset_rfl
...
/-- `Yseq X Δ n` is always either empty or a genuine `U`-neighbourhood: it is a finite union
(`Fin n → Bool`) of `atomU` pieces, each of which is `∅` or `U.mem` by `atomU_invariant`. -/
theorem Yseq_empty_or_mem (n : ℕ) : Yseq X Δ n = ∅ ∨ U.mem (Yseq X Δ n) := by
  by_cases hne : (Yseq X Δ n).Nonempty
  · exact Or.inr (U_iUnion_mem
      (fun δ' => (atomU_invariant X Δ hΔ (n + 1)).2.1 (extendTrue δ')) hne)
  · exact Or.inl (Set.not_nonempty_iff_eq_empty.mp hne)

/-- If `x ∈ X n` (and `x ∈ Δ`, automatic whenever `X n ⊆ Δ`), then `Yseq X Δ n` is non-empty: the
canonical sign sequence `δ₀ k := decide (x ∈ X k)` produces a depth-`n` atom containing `x` (hence
non-empty), which `X n` (since `x ∈ X n`) splits into a non-empty `"+"`-piece that is, by
`splitChoice_fst_eq_inter_Yseq`, a subset of `Yseq X Δ n`. -/
theorem Yseq_nonempty_of_mem {n : ℕ} {x : α} (hxΔ : x ∈ Δ) (hxn : x ∈ X n) :
    (Yseq X Δ n).Nonempty := by
  classical
  ...
  obtain ⟨hmatch, hmem, -⟩ := atomU_invariant X Δ hΔ n
  have hspec := splitChoice_spec (hmatch δ0) (hmem δ0) (X n)
  have hIne : (splitChoice (genAtom X Δ δ0 n) (atomU X Δ δ0 n) (X n)).1 ≠ ∅ :=
    fun h => hAne.ne_empty (hspec.2.2.1.mpr h)
  have hI_eq_atomU : (splitChoice (genAtom X Δ δ0 n) (atomU X Δ δ0 n) (X n)).1 =
      atomU X Δ δ0 (n + 1) := by rw [atomU_succ, hδ0n, if_pos rfl]
  ...
  have hδ2_eq : atomU X Δ δ2 (n + 1) = atomU X Δ δ0 (n + 1) := atomU_congr X Δ hagree
  have hne2 : (atomU X Δ δ2 (n + 1)).Nonempty := by
    rw [hδ2_eq, ← hI_eq_atomU]
    exact Set.nonempty_iff_ne_empty.mpr hIne
  exact hne2.mono (subset_Yseq X Δ (restrictFin δ0 n))
