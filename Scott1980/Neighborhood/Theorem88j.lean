...
* **`D_X_surj`** — every `fixedNbhd a`-neighbourhood `Y` is hit by some `D_X qChar cons c`: given
  `Y ∈ fixedNbhd a`, `P.surj` produces a raw `V`-index `n₀` with `P.X n₀ = Y`; `Y` being
  `a`-fixed gives `DiagFixed P a n₀`, and Part 1's `hqChar` extracts a witness `i₀` with
  `qChar ⟨i₀, n₀⟩ = 1`. Feeding the **singleton list `[⟨i₀, n₀⟩]`** (encoded via `encodeList`) into
  the fold performs exactly one step from `P.masterIdx`: the `qChar`-gate passes by construction,
  and the `V`-consistency gate passes because `P.X P.masterIdx = V.master ⊇ Y` (`V.sub_master`)
  makes `P.masterIdx`/`n₀` trivially consistent via the witness `n₀` itself
  (`P.X n₀ ⊆ P.X P.masterIdx ∩ P.X n₀`, in fact equality). The fold therefore advances to
  `P.inter P.masterIdx n₀`, and `P.inter_spec` gives `P.X (P.inter P.masterIdx n₀) =
  V.master ∩ Y = Y`.
...
`D_X qChar cons c` for some list-code `c`: the singleton-list code witnessing `Y`'s raw `V`-index
together with Part 1's `qChar`-witness for it. -/
theorem D_X_surj {a : ApproximableMap V V}
  ...
  obtain ⟨i₀, hi₀⟩ := (hqChar n₀).mp hdiag
  refine ⟨encodeList [Nat.pair i₀ n₀], ?_⟩
  have hstep : myFoldCode P qChar cons (encodeList [Nat.pair i₀ n₀])
      = myStep P qChar cons P.masterIdx (Nat.pair i₀ n₀) := by
    rw [myFoldCode_eq, decodeList_encodeList]
    unfold myFold
    rw [List.foldl_cons, List.foldl_nil]
  have hcond : cons (Nat.pair P.masterIdx n₀) = 1 := by
    apply (hcons P.masterIdx n₀).mpr
    refine ⟨n₀, ?_⟩
    have hsub : Y ⊆ V.master := V.sub_master hYV
    rw [P.masterIdx_spec, hn₀]
    exact Set.subset_inter hsub subset_rfl
  have hstepval : myStep P qChar cons P.masterIdx (Nat.pair i₀ n₀)
      = P.inter P.masterIdx n₀ := by
    unfold myStep
    simp only [unpair_pair_snd]
    rw [(isOne_eq_one_iff _).mpr hi₀, (isOne_eq_one_iff _).mpr hcond, Nat.one_mul, selectFn_one]
  show D_X P qChar cons (encodeList [Nat.pair i₀ n₀]) = Y
  unfold D_X
  rw [hstep, hstepval]
  have hcons' : (∃ k, P.X k ⊆ P.X P.masterIdx ∩ P.X n₀) := (hcons P.masterIdx n₀).mp hcond
  rw [P.inter_spec hcons', P.masterIdx_spec, hn₀, Set.inter_eq_right.mpr (V.sub_master hYV)]
